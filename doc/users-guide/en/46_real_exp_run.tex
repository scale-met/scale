...
According to the above setting,  variables listed in the following \nmitem{HISTORY_ITEM} are output.  It is possible to change output intervals at every variable in \nmitem{HISTORY_ITEM} by adding options if needed.  The mean value instead of the snapshot value can be output.  Refer to Section \ref{sec:output} for the details.
\editboxtwo{
