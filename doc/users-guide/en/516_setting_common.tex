...
The performance profiler is specified in \namelist{PARAM_PROF} in files \verb|init.conf| and \verb|run.conf|.
...
%
Each measurement section has the output level. The sections larger than the \nmitem{PROF_rap_level} are not measured.
...
The statistics monitor is specified in \namelist{PARAM_STATISTICS} in files \verb|init.conf| and \verb|run.conf|.
