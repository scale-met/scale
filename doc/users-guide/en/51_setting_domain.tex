...
The numbers of grids per MPI process are specified by \nmitem{IMAX, JMAX} in \namelist{PARAM_ATMOS_GRID_CARTESC_INDEX}.
...
\subsubsection{Setting vertical grids for urban model}
The number of vertical layers for urban model is specified by \nmitem{UKMAX} in \namelist{PARAM_URBAN_GRID_CARTESC_INDEX}.
The vertical grid intervals are specified by \nmitem{UDZ} in \namelist{PARAM_URBAN_GRID_CARTESC}. The unit is [m].
\editboxtwo{
\verb|&PARAM_URBAN_GRID_CARTESC_INDEX| & \\
\verb| UKMAX = 5,|  & ; number of vertical layers \\
\verb|/|\\
\\
\verb|&PARAM_URBAN_GRID_CARTESC| & \\
\verb| UDZ = 0.01, 0.01, 0.03, 0.05, 0.10,| & ; grid interval along the vertical direction\\
\verb|/|\\
}
You can specify an array in \nmitem{UDZ} for the number of vertical layers specified by \nmitem{UKMAX}.
The order of the array is from the ground surface to the atmosphere.
