...
\editboxtwo{
  ...
  \verb| ATMOS_BOUNDARY_IN_CHECK_COORDINAMTES | \textbackslash \\
  ...
  \verb|                    = 'lerp_initpoint',| & ; Temporal interpolation type. \verb|same_parent|: use the latest step value (no interpolation), \verb|nearest_neighbor|: use the value of the nearest time step, \verb|lerp_initpoint|: linear interpolation between values at two time steps as the values are snapshot, \verb|lerp_midpoint|: same as \verb|lerp_initpoint| but the values are temporal average during the time step. \\
...
The nudging is applied as
\begin{eqnarray}
  \frac{\partial \phi_{k,i,j}}{\partial t}
...
\begin{equation}
  \alpha_z =  \left\{
    \begin{array}{lll}
      ( 0.5 - \cos(\pi e_z)/2 ) / \tau_z & \mathrm{for} & e_z \le 0.5, \\
      ( 0.5 + \sin(\pi (e_z-0.5))/2 ) / \tau_z & \mathrm{for} & e_z > 0.5,
    \end{array} \right.
\end{equation}
...
The $\tau$ is the relaxation time in which the difference between the simulated value and the boundary value becomes $1/e$.
The default value is ten times of \nmitem{TIME_DT}. 
Please refer to Section \ref{sec:timeintiv} for \nmitem{TIME_DT}.


If \nmitem{ATMOS_BOUNDARY_TYPE} in \namelist{PARAM_ATMOS_BOUNDARY} is \verb|REAL|, the nudging at the lateral boundary for the horizontal velocities, potential temperature, and vapor is applied.
