...
\editboxtwo{
  ...
  \verb| ATMOS_BOUNDARY_DENS_ADJUST = .false., | & ; Switch of the density adjustment. \\
  \verb| ATMOS_BOUNDARY_DENS_ADJUST_TAU | \textbackslash \\
  ~~\verb|     = -1.0D0,                       | & ; Time scale of the density nudging when the density adjustment is enabled (in second). \\
}
...
If \nmitem{ATMOS_BOUNDARY_TYPE} = ``\verb|REAL|'' in \namelist{PARAM_ATMOS_BOUNDARY}, the nudging at the top and lateral boundaries for the horizontal velocities, potential temperature, density, and vapor is applied.
In addition, the vertical velocity and hydrometeors can be nudged by set \nmitem{ATMOS_BOUNDARY_USE_VELZ} and \nmitem{ATMOS_BOUNDARY_USE_QHYD} to \verb|.true.|, respectively.
If the simulation is on a child domain of the online nesting (See Section \ref{subsec:nest_online}), the same setting is applied with the \verb|REAL| boundary type but \nmitem{ONLINE_USE_VELZ} and \nmitem{ONLINE_BOUNDARY_USE_QHYD} in \namelist{PARAM_COMM_CARTESC_NEST} instead of \nmitem{ATMOS_BOUNDARY_USE_VELZ} and \nmitem{ATMOS_BOUNDARY_USE_QHYD}, respectively.

The density nudging reduces the pressure gradient, which plays a role to transfer the information of boundary data into the inner domain especially in outflow region, although it is necessary to reduce total mass bias in the simulation.
The density nudging could be weaken by adjusting the mass flux at the lateral boundaries, which can also reduce the mass bias.
If \nmitem{ATMOS_BOUNDARY_DENS_ADJUSTMENT} = \verb|.true.|, the mass flux is adjusted so that the difference in the total mass between the parent model and the simulation becomes small.
...
When the density adjustment is enabled, the time scale of the density nudging is specified by \nmitem{ATMOS_BOUNDARY_DENS_ADJUSTMENT_TAU}.
