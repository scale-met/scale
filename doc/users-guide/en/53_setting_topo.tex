...
The configurations for the conversion are specified in \namelist{PARAM_CNVTOPO} as follows:
\editboxtwo{
\verb|&PARAM_CNVTOPO                               | & \\
...
As external topography data, \scalerm supports GTOPO30 provided by the U.S. Geological Survey and DEM50M provided by the Geospatial Information Authority of Japan.
...
User can use the parameter \nmitem{CNVTOPO_name} to specify which data to be used, instead of the switches;
...
To use the GTOPO30 data, directory where the data is stored and the path of the catalogue file must be specified by \nmitem{GTOPO30_IN_DIR} and \nmitem{GTOPO30_IN_CATALOGUE} in \namelist{PARAM_CNVTOPO_GTOPO30}, respectively.
...
In the same manner, for the DEM50M data, \nmitem{DEM50M_IN_DIR} and \nmitem{DEM50_IN_CATALOGUE} in \namelist{PARAM_CNVTOPO_DEM50M} is need to be set to \\ \verb|$SCALE_DB/topo/DEM50M/Products| and \verb|$SCALE_DB/topo/DEM50M/Products/DEM50M_catalogue.txt|, respectively.
Configurations for the user-defined data are explained in Section \ref{subsec:topo_userfile}.

There are two types of filter for the smoothing the elevation with the steep slope in interpolated topography: Laplacian and Gaussian filters.
...
Instead of \nmitem{CNVTOPO_smooth_maxslope_ratio}, you can directly set the maximum angle $\theta_{\max}$ by setting the parameter \nmitem{CNVTOPO_smooth_maxslope} in degree.
