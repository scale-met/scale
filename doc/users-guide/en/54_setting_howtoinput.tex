...
The ``SCALE format'' is mainly used for experiments with offline nesting.
...
Refer to Section \ref{sec:tutorial_real_data} for details.
...
If other format data, such as GRIB/GRIB2 data is converted to the binary format, \scalerm can read the data. 
...
For example, in the tutorail described in section\ref{sec:tutorial_real_intro}, \nmitem{RESTART_OUT_BASENAME} = '\verb|init_d01|'. 
These setting mentioned above are configured when you want to output restart files in executing \scalerm (see section\ref{sec:restart} for details). 
The structure of generated restart files is same as that of initial files. 

The settings for initial and boundary files are specified by \namelist{PARAM_MKINIT_REAL_(ATMOS|OCEAN|LAND)} in the file \verb|init.conf|. 
%%
\editboxtwo{
...
The above configurations except for \nmitem{BASENAME_BOUNDARY} can be shared among \verb|ATMOS| and \verb|OCEAN|(or \verb|LAND|); 
...
For the default setting where both of values are \verb|.false.|, the density in initial and boundary data is calculated with hydrostatic balance, i.e., $\frac{dp}{dz}=-\rho g$ from the input datat of temperature and humidity.
...
\verb| INIT_LANDWATER_RATIO = 0.5    | & ; (For \verb|USE_FILE_LANDWATER=.false.|) degree of saturation\\
\verb|  ..........                   | & \\
\verb|/| & \\
}
...
The namelist file specified by \nmitem{BASENAME_ORG} needs to be prepared beforehand by users. 
...
If the number of grids for a variable is different from the default value, the value is specified by \namelist{GrADS_ITEM}.
...
For \nmitem{upper_qv_type}='\verb|COPY|', the value of RH at layers higher than \verb|nz| is set to that at the top layer where input humidity data exists;  
It determine the value of QV. 
