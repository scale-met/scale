...
\begin{table}[bth]
\begin{center}
\caption{Variable list of \nmitem{item} in \namelist{grdvar}. The asterisk means ``it is optional but recommended as possible''. The double-asterisk means ``it is available but not recommended''. When you target on the place at high altitude, use of \texttt{HGT} is strongly recommended.}
