...
If \nmitem{USE_FILE_DENSITY} = \verb|.true.|, the input density is used for the density as well as the other variables.\\
...
\nmitem{USE_SFC_DIAGNOSES} is switch for the calculation of the values at the layers below the lowermost grid of the input data.
...
There are two options in preparation of soil moisture: i) to provide the data from the input file (\nmitem{USE_FILE_LANDWATER} = \verb|.true.|) and ii) to provide it as a constant value in the entire region (\nmitem{USE_FILE_LANDWATER} = \verb|.false.|).
