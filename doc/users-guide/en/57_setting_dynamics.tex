...
For advection scheme of prognostic variables in dynamics (\nmitem{ATMOS_DYN_FVM_FLUX_TYPE}), 
the default setting is 4th-order central difference (\verb|CD4|) in the \scalerm.
When using \verb|CD4| in an experiment with topography,
it is often confirmed that grid-scale convection is occurred on the peak of mountains.
This grid-scale convection is moderated by using \verb|UD3|. 
So, the use of \verb|UD3| is recommended for experiments with topography.
