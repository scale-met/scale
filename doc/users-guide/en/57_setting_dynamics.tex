...
\nmitem{ATMOS_DYN_NUMERICAL_DIFF_COEF} is for the dynamical prognostic variables, such as density, momentum and potential temperature, \\
...
During simulation, the reference state is updated at an interval of \nmitem{ATMOS_REFSTATE_UPDATE_DT} if it is set to a positive value.
...
By updating the reference state at an appropriate interval, the reference state will be identical; \nmitem{ATMOS_REFSTATE_UPDATE_DT} should be a divisor of the restart interval.
Alternatively, you can use the same reference state by outputing the state to a file specified by \nmitem{ATMOS_REFSTATE_OUT_BASENAME} in the first simulation run and inputing the state from the file by specifing \nmitem{ATMOS_REFSTATE_IN_BASENAME} in the following runs.
