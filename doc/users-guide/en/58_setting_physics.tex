...
\begin{center}
  ...
  \begin{tabularx}{150mm}{lXX} \hline
    \rowcolor[gray]{0.9}  Value & Description of scheme & Reference\\ \hline
      \verb|OFF|  & Do not calculate the cumulus parameterization &  \\
...
Utilization of cumulus paramterization is strongly recommended when grid spacing is more than 5 km to avoid calculation of unnatual strong deep convections.
The precipitation from cumulus parameterization and micro physics are outputted to the variables \verb|RAIN_CP| and \verb|RAIN_MP|, individually. In addtion, precipitation from cumulus parameterization is summed in the variable \verb|RAIN| (i.e. \verb|RAIN| = \verb|RAIN_CP| + \verb|RAIN_MP|). This is same as the variable \verb|PREC|.
...
The time interval of the calling KF specified \nmitem{TIME_DT_ATMOS_PHY_CP} in \namelist{PARAM_TIME} is also tuning parameter effective to the precipitation amount. For the first setting, recommend 300s for \nmitem{TIME_DT_ATMOS_PHY_CP}.
...
Gases considered in radiation calculation are carbon dioxide($CO_{2}$), ozone($O_{3}$), Nauru's oxide($N_{2}O$), carbon monoxide($CO$), methane($CH_{4}$), oxigen($O_{2}$), and chlorofluorocarbons($CFCs$). These concentrations are able to be set to zero in \namelist{PARAM_ATMOS_PHY_RD_PROFILE}.
