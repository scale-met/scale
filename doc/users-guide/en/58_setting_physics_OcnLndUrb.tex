...
If \nmitem{OCEAN_DYN_TYPE} is set to \verb|"SLAB"|, the subsurface ocean is treated as a slab layer. The temperature of the slab layer develops over time by the heat flux from the surface. The depth of the slab layer, which controls the heat capacity of the slab ocean, can be specified \nmitem{OCEAN_DYN_SLAB_DEPTH} in \namelist{PARAM_OCEAN_DYN_SLAB}.
...
\editboxtwo{
 ...
 \verb| OCEAN_DYN_OFFLINE                       = .false., | & ; use nudging for ocean variables? \\
 ...
 \verb| OCEAN_DYN_OFFLINE_enable_periodic_day   = .false., | & ; dayly cyclic data? \\
...
If \nmitem{OCEAN_ALB_TYPE} is set to \verb|"CONST"| or \verb|"NAKAJIMA00"|, the constant albedo is used for the long-wave (IR).
...
For the short-wave (visible and near-IR band), the albedo parameters above are used when \nmitem{OCEAN_ALB_TYPE} is set to \verb|"CONST"|.
If \nmitem{OCEAN_ALB_TYPE} is set to \verb|"NAKAJIMA00"|, the albedo for the short-wave is calculated from the solar zenith angle.
The albedos over the sea ice surface are constant. The values are set by \nmitem{PARAM_OCEAN_PHY_ALBEDO_seaice}.
...
The roughness lengths of the sea ice surface are constant. \\
...
By using the albedo and the roughness length, the surface fluxes between atmosphere and ocean, and fluxes between atmosphere and ice are calculated by the scheme selected by \nmitem{OCEAN_SFC_TYPE}. The bulk scheme specified by \nmitem{BULKFLUX_TYPE} in \namelist{PARAM_BULKFLUX} is used for this calculation. Refer to Section \ref{sec:basic_usel_surface} for more detail of bulk scheme.
