...
The sample file for \nmitem{URBAN_DYN_KUSAKA01_PARAM_IN_FILENAME} is provided at \\
\verb|scale/scale-rm/test/data/urban/param.kusaka01.dat|.
...
\verb|scale_init| is used to prepare the gridded urban data; the detailed explanations are given in Section x.x.
Here, only a practical usage is demonstrated with examples of configuration file and namelist file.
...
Below shows an example of \verb|init.conf| in the case of preparing AH gridded data.
...
Below shows an example of namelist file, corresponding to ``ctl'' file of \grads.
\editbox{
\verb|#              | \\
\verb|# Dimension    | \\
\verb|#              | \\
...
while the Z0M data contains only one data because Z0M is assumed to unchanged during the simulation
(Namely, \nmitem{CNVUSER_NSTEPS}=1).
\verb|tdef| of ``ctl'' corresponds to \nmitem{CNVUSER_NSTEPS} of \namelist{PARAM_CNVUSER} in \scale.
