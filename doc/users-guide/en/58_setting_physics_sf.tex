...
%-------------------------------------------------------------------------------
If \nmitem{ATMOS_PHY_SF_TYPE} $=$ \verb|"BULK"|, the surface flux is calculated by the bulk model using the surface temperature and roughness lengths, which are given as constant values. Evaporation efficiency can also be given optionally in a range of 0 to 1. This flexibility enables the ideal experiment not only for ocean surfaces but also for land. The evaporation efficiency is specified in \nmitem{ATMOS_PHY_SF_BULK_beta} in \namelist{PARAM_ATMOS_PHY_SF_BULK} in the file \verb|run.conf| as follows:
