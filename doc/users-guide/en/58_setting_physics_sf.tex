...
The time step to call the surface flux scheme is configured in \nmitem{TIME_DT_ATMOS_PHY_SF} and \nmitem{TIME_DT_ATMOS_PHY_SF_UNIT} in \namelist{PARAM_TIME}.
...
If \nmitem{ATMOS_PHY_SF_FLG_MOM_FLUX} is 0, a constant bulk coefficient is used to calculate the surface momentum flux; its value is specified by \nmitem{ATMOS_PHY_SF_Const_Cm}. \\
If \nmitem{ATMOS_PHY_SF_FLG_MOM_FLUX} is 1, a constant friction velocity is used to calculate the bulk coefficient; its value is specified by \nmitem{ATMOS_PHY_SF_Const_Ustar}.
...
These parameters can be set in \namelist{PARAM_ATMOS_PHY_SF_VARS} of \verb|run.conf|.
%
\editboxtwo{
\verb|&PARAM_ATMOS_PHY_SF_VARS                      | & \\
\verb| ATMOS_PHY_SF_DEFAULT_SFC_TEMP      = 300.0,  | & ; Surface temperature \\
\verb| ATMOS_PHY_SF_DEFAULT_SFC_albedo_LW = 0.04,   | & ; Albedo for long-wave radiation \\
\verb| ATMOS_PHY_SF_DEFAULT_SFC_albedo_SW = 0.10,   | & ; Albedo for short-wave radiation \\
\verb| ATMOS_PHY_SF_DEFAULT_SFC_Z0M       = 1E-4,   | & ; Roughness length for momentum  \\
\verb| ATMOS_PHY_SF_DEFAULT_SFC_Z0H       = 1E-5,   | & ; Roughness length for heat  \\
\verb| ATMOS_PHY_SF_DEFAULT_SFC_Z0E       = 1E-5,   | & ; Roughness length for vapor  \\
\verb|/                                             | & \\
}
