...
\begin{center}
  ...
  \begin{tabularx}{150mm}{lXX} \hline
    ...
    scale\_cartesC\_prc\_rank\_[xy] & mapping index of the 2D decomposition & \verb|PRC_2Drank(PRC_myrank,1 or 2)| in the model \\
    scale\_cartesC\_prc\_num\_[xy] & number of the 2D decomposition & \nmitem{PRC_NUM_X}, \nmitem{PRC_NUM_Y} in the model \\
    scale\_cartesC\_prc\_periodic\_[zxy] & wheather the boundary condition is periodic & 0 for .false. and 1 for .true, 0. They correspoind to \nmitem{PRC_PERIODIC_X}, \nmitem{PRC_PERIODIC_Y} in the model \\
    \shortstack{scale\_atmos\_grid\_cartesC\\\_index\_[ij]maxg} & number of the grids in the global domain & \nmitem{IMAX} $\times$ \nmitem{PRC_NUM_X}, \nmitem{JMAX} $\times$ \nmitem{PRC_NUM_Y} in the model \\
    \shortstack{scale\_atmos\_grid\_cartesC\\\_index\_kmax} & number of the vertical layer for the atmospheric model & \nmitem{KMAX} in the model \\
    \shortstack{scale\_ocean\_grid\_cartesC\\\_index\_kmax} & number of the vertical layer for the ocean model & \nmitem{OKMAX} in the model \\
    \shortstack{scale\_land\_grid\_cartesC\\\_index\_kmax} & number of the vertical layer for the land model & \nmitem{LKMAX} in the model \\
    \shortstack{scale\_urban\_grid\_cartesC\\\_index\_kmax} & number of the vertical layer for the urban & \nmitem{UKMAX} in the model \\
    \shortstack{scale\_atmos\_grid\_cartesC\\\_index\_[kij]halo} & number of halo grids & \nmitem{KHALO}, \nmitem{IHALO}, \nmitem{JHALO} in the model \\
    calendar & calendar type & \nmitem{PARAM_CALENDAR} in the model \\
    time\_start, time\_units & init time \\ \hline
  \end{tabularx}
\end{center}
\end{table}
...
Section \ref{sec:domain} for \nmitem{PRC_NUM_X, PRC_NUM_Y, PRC_PERIODIC_X, PRC_PERIODIC_Y, KMAX, IMAX, JMAX},
and \ref{subsec:calendar} for \nmitem{PARAM_CALENDAR}.
...
The ``cell\_measures'' attribute of each variables specifies the correspoinding area or volume data.

The map projection data is contained as a non-dimentional variable whose name is specified by ``grid\_mapping'' attribute of the variables.

The information of relationship the staggared grids, an attribute and a non-dimensional variable are contained as follows SGRID conventions (\url{https://github.com/sgrid/sgrid}).
The non-dimensional variable's name is specified by the ``grid'' attribute of each variable.

In the file, there also the surface elevation data and land mask data as ``topo'' and ``lsmask'', respectively.
...
As default, all the data files are output by each process, namley separate file I/O.
If \scalerm is compiled with pnetCDF (SCALE\_ENABLE\_PNETCDF=T), the data from all processes can be combined into a single file (See Section\ref{sec:scale_compile}).
To do this, set \nmitem{FILE_AGGREGATE} in \namelist{PARAM_FILE}) .true..
The all the data files become single file.
