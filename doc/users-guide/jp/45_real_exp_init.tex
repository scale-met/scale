...
\subsection{大規模並列計算におけるメモリ節約}

SCALEでは、初期値データの読込はマスタープロセスのみが行い、broadcast通信によって各ノードに情報を伝播する。
これはできるだけファイルIOを減らし、高速に初期値データを処理するためのアルゴリズムである。
一方でこのアルゴリズムは、大規模並列計算システムで大きな初期値データを読み込むと、メモリ容量が足りなくなることがある。
これを回避するために、各ノードが自身に必要なデータだけを読み込む方式に切り替え、メモリ不足を解消することができる。
...
デフォルト設定は.true.であり、マスタープロセスのみファイルIOを実行する。
.false.にすると、各ノードがファイルIOを行うことで使用するメモリ量を削減する。
