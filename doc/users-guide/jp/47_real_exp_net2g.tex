...
プログラム\verb|netcdf2grads (略して、net2g)|は、プロセス毎に分割された{\netcdf}ファイル(\verb|history.**.nc|
...
変換した{\grads}バイナリデータを使って、シミュレーション結果の確認を行う。
...
シミュレーションを実行する時に用いたプロセス数の約数である必要がある。
ここでは、4プロセスを使用する。
...
成功すれば、下記のファイルが作成される。
\verb|**.ctl|は {\scalerm} のXY格子座標系のctlファイル、
