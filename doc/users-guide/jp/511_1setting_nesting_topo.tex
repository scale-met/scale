...
ここでは、親領域の地形の出力データが SCALE netCDF フォーマット で \verb|topo_d01.pe***.nc|として保存されていると想定する。その他のフォーマットとしては、GrADS および WRF-ARW がサポートされている（第\ref{sec:adv_datainput}節）。
\editboxtwo{
\verb|&PARAM_NEST| & \\
...
\verb| COPYTOPO_IN_FILETYPE   = "SCALE",   | & 親領域の地形データファイルのタイプ名 (SCALE, GrADS, or WRF-ARW) \\
