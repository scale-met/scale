...
\item 親領域の時間ステップは子領域の時間ステップの倍数でなければならない。
\end{itemize}
一方、親領域と子領域において鉛直層数、鉛直レベル、地図投影法、物理スキームが一致している必要はない。
...
USER.shに名前を変更して、「実験セット準備ツール」を実行することによって作成される
...
親領域で計算された凝結物を子領域の境界条件として与えることが可能である。
この改善策によって、流入境界付近での雲や降水の生成の遅れの影響を小さくすることが期待される。
...
例えば、「実験セット準備ツール」によって用意した設定ファイルでは、
...
このようなメッセージが出た場合には、地形・土地利用データ、および初期値/境界値の作成から
...
これを避けるためには、各プロセスにおける時間積分の計算量をプロセス間で可能な限り揃うように、
MPIプロセスを配置すると良い\footnote{正確を期すなら演算量を見積もる必要がある。}。
...
その理由は、このような設定によって HALO領域を減らせるためである。
...
上記の説明では、格子点数と積分時間のタイムステップのみを考慮した。
しかし、現実大気のネスティング実験のような実際の計算では、
各領域における物理過程の時間間隔の違いや、
