...
減衰の時定数は、大気上端で最も小く、スポンジ層下端で無限大となる。
...
\begin{enumerate}
\item 下限高度の指定\\
  \nmitem{ATMOS_DYN_wdamp_height} でスポンジ層の下限高度を指定する。指定された高度よりも上がスポンジ層となる。単位は m である。
\item 層数の指定\\
  \nmitem{ATMOS_DYN_wdamp_layer} でスポンジ層の層数を指定する。上方の指定された層数がスポンジ層となる。
\end{enumerate}
両方が指定されなかった場合は、鉛直速度の減衰は行われない。また、両方が指定された場合は、\nmitem{ATMOS_DYN_wdamp_layer} の設定が採用される。

スポンジ層上端での時定数は、\nmitem{ATMOS_DYN_wdamp_tau} で設定する。単位は秒である。\\
\nmitem{TIME_DT_ATMOS_DYN}よりも小さい値を設定することは出来無い。
