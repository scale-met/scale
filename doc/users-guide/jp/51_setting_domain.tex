...
すなわち、MPIプロセス1つあたりの水平格子数は\namelist{PARAM_ATMOS_GRID_CARTESC_INDEX}の\nmitem{IMAX, JMAX}で指定され、
...
\verb| UDZ = 0.01, 0.01, 0.03, 0.05, 0.10,| & ; 鉛直方向の格子間隔\\
\verb|/|\\
}
\nmitem{UDZ}には\nmitem{UKMAX}で指定した格子数分の配列を指定する。
配列の順序は、地表面から上空に向かう方向である。
