...
水平ー鉛直の2次元実験を行うためには、\texttt{IMAX} および \texttt{PRC\_NUM\_X} を 1 に設定する。
つまり、X方向の全格子数を 1 とする。
...
\editboxtwo{
  ...
  \verb| ATMOS_BOUNDARY_DENS_ADJUST = .true.,  | & ; 密度調整に対するスイッチ. \\
  \verb| ATMOS_BOUNDARY_DENS_ADJUST_TAU | \textbackslash \\
  ~~\verb|     = -1.0D0,                       | & ; 密度調整が有効な場合の密度ナッジングの時定数 (秒). \\
}
...
境界の種類として\verb|REAL|を用いた場合と同様の設定が適用される。
この場合、\namelist{PARAM_COMM_CARTESC_NEST} 内の \nmitem{ONLINE_USE_VELZ} および \nmitem{ONLINE_BOUNDAYR_USE_QHYD} がそれぞれ\nmitem{ATMOS_BOUNDARY_USE_VELZ} と \nmitem{ATMOS_BOUNDARY_USE_QHYD} の代わりに用いられる。

密度ナッジングは質量バイアスを軽減するために必要である。
一方で、気圧傾度力は特に流出域において境界データ情報をドメイン内部に伝える役割をもっているが、密度ナッジングはその気圧傾度力を減少させてしまう。
質量バイアス軽減させるように側面境界における質量フラックスを調整することで、密度ナッジングを弱くすることができる。
\nmitem{ATMOS_BOUNDARY_DENS_ADJUST} を \verb|.true.| とすると、親モデルとシミュレーションにおける全領域総質量の差が小さくなるように、側面境界質量フラックスが調整される。
