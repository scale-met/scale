...
\end{eqnarray}
と決定される。ここで、\nmitem{DX, DY}は、\ref{subsec:gridinterv}節に記述されるように、
\namelist{PRAM_ATMOS_GRID_CARTESC}内で指定する。
水平方向の解像度と領域の大きさが設定され、またMPIプロセス数が与えられるならば、
上記の関係から 1 つのMPIプロセスが受け持つ格子点数は決まる。

次の小節では、MPIプロセス数、格子数、格子間隔の設定をより詳しく説明する。
\textcolor{red}{これらの設定は\texttt{pp.conf}、\texttt{init.conf}、\texttt{run.conf}の設定ファイル間で一致させなければならない}
ことに注意が必要である。
...
総MPIプロセス数は、\verb|PRC_NUM_X| $\times$ \verb|PRC_NUM_Y|  によって与えられる。
上記の例では、領域が\XDIR には2分割、\YDIR には分割なしの 2-MPI並列ということになる。
この総プロセス数は、ジョブを投げる際のMPIコマンドに指定するMPIプロセス数として与える必要がある。
...
詳細は、図\ref{fig:scale_grid}を参照されたい。
また、\nmitem{FZ(:)}で指定する要素の数は、鉛直層数
（\namelist{PARAM_ATMOS_GRID_CARTESC_INDEX}内の\nmitem{KMAX})と一致している必要があることに注意されたい。
...
標高がある場所での鉛直格子点の位置は、地形に沿った座標系によって適切に取り扱われる。
...
スポンジ層内では、鉛直速度をレイリー摩擦によって減衰させる。
緩和の時定数(e-folding time)は、モデル上端で最も短く、高度が下がるにつれて長くする。
スポンジ層の下端より下では、緩和の時定数は無限大に設定する。
スポンジ層の厚さは、\namelist{PARAM_ATMOS_DYN}において以下のどちらかの方法で行う。
\begin{enumerate}
\item スポンジ層の層数を指定\\
  \nmitem{ATMOS_DYN_wdamp_layer} でスポンジ層の層数を指定する。
  その層数はモデル上端から数える。
\item スポンジ層の下端高度[m]の指定\\
  \nmitem{ATMOS_DYN_wdamp_height} で指定された高度よりも上部にある層をスポンジ層として設定する。
\end{enumerate}
...
実際の計算から得られる出力データの間に、値の不一致が起こる。
計算において、この乖離は非物理的なモード等の幾つかの問題を生じさせる。
%両者の乖離が大きくなると、境界から流入する情報を計算領域内にうまく取り込むことができない他、
%音波や重力波の非物理的な生成や反射が起る。
この問題を回避するために、領域に「緩和領域」を設ける。

図\ref{fig:buff_xz}に示されるように、\scalerm では計算領域のすぐ内側に緩和領域を設置する。
緩和領域では、境界値データや親モデルのデータによって指定した値にある時定数で近づけるために、
...
\nmitem{FZ(:)}を与えることによって鉛直レベルを指定した場合
(第\ref{subsec:gridinterv}節参照)には、上記のストレッチの設定は{\ZDIR}には適用されない。
...
\editboxtwo{
  ...
  \verb| ATMOS_BOUNDARY_TYPE = 'NONE',         | & ; 境界データの種類。表\ref{tab:nml_atmos_boundary_type}を参照。 \\
  \verb| ATMOS_BOUNDARY_IN_BASENAME = '',      | & ; 境界データのファイル名 (\verb|OFFLINE| または\verb|REAL| type の場合)\\
  ...
  ~~\verb|        = (/ -9999, 0, 0, 0, 0, 0 /),| & ; 境界データの開始時刻(\verb|REAL| type の場合のみ) \\
  \verb| ATMOS_BOUNDARY_UPDATE_DT = 0.0D0,     | & ; 境界データの時間間隔(\verb|REAL| type の場合のみ) \\
  ...
  \verb| ATMOS_BOUNDARY_OUT_BASENAME = '',     | & ; 初期境界データを出力するファイル名 \\
...
\namelist{PARAM_ATMOS_BOUNDARY}内の\nmitem{ATMOS_BOUNDARY_TYPE}が\verb|REAL|であれば, 水平速度・温位・水蒸気に対する側面境界でのナッジングは、
強制的に適用される。
...
計算がオンラインネスティング(第\ref{subsec:nest_online}節を参照)の子領域で行われるとき、
境界の種類として\verb|REAL|を用いて、上記と同様の設定が適用される。

上端境界の周辺では同様な減衰のさせ方として、レイリー摩擦が存在する(第 \ref{subsec:raydamp}節を参照)。
