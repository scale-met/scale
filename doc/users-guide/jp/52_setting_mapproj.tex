...
南側、北側の標準緯線はそれぞれ\nmitem{MAPPROJECTION_LC_lat1, MAPPROJECTION_LC_lat2}で指定する(単位は[度])。
両標準緯線に挟まれた領域では、経線に対する緯線の長さの比が、地球の楕円体面上での長さの比と近くなるように調節される。

さらに下記のように設定すれば、基準点(\nmitem{MAPPROJECTION_basepoint_x, MAPPROJECTION_basepoint_y})を、
...
\nmitem{MAPPROJECTION_basepoint_x, MAPPROJECTION_basepoint_y}はそれぞれ、
