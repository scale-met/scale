...
外部標高データとして、\scalerm は U.S. Geological Survey が提供する GTOPO30 および 国土地理院が提供する GEM50M をサポートしている。
...
\nmitem{CNVTOPO_name} の値にしたがって、\nmitem{CNVTOPO_(UseGTOPO30|UseDEM50M|UseUSERFILE)} は表\ref{tab:cvntopo_name}で示されている値に設定される。
...
同様に、DEM50Mデータについては、\nmitem{DEM50M_IN_DIR} および \\ \nmitem{DEM50_IN_CATALOGUE} に、それぞれ \verb|$SCALE_DB/topo/DEM50M/Products| および \\ \verb|$SCALE_DB/topo/DEM50M/Products/DEM50M_catalogue.txt| となる。
