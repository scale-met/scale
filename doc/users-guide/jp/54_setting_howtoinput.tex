...
プログラム\verb|scale-rm_init|では、\verb|00000|から\nmitem{NUMBER_OF_FILES}-1までの数字が付けられたファイルを読み込む。
...
\nmitem{BASENAME_BOUNDARY}を指定しなければ、 境界ファイルは出力されない。
...
これらの情報は引き継がれる。
%
\nmitem{USE_FILE_DENSITY}は\verb|FILETYPE_ORG="SCALE-RM"|としたときのオプションであり、
