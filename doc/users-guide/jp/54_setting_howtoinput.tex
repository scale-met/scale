...
\scalerm では、表\ref{tab:inputdata_format}に示される様々な種類の外部データから、初期値・境界値データを作成可能である。
...
バイナリ形式は、Fortran が直接アクセスできる単精度浮動小数点のバイナリデータである。
詳細については、第\ref{sec:tutrial_real_data}節で説明する。

WRFデータ形式も使用でき、WRFによるモデル出力データを直接使用できる。
ただし、ファイルは{\scalerm}の境界値データの作成に必要な全てのデータを含まなければならない。

その他のフォーマットデータ（例えば、GRIB/GRIB2 データなど）は、バイナリデータ形式に変換することで {\scalerm} に読み込ませることができる。
...
（初期値ファイルとリスタートファイルは同じネームリストを使用し、作成されたファイルは同じ構造を持つ（リスタートに関する説明は 第\ref{sec:restart}章参照））
%
\editboxtwo{
\verb|&PARAM_RESTART|                      & \\
...
\verb| USE_FILE_DENSITY           = .false.,|                        & ; 入力ファイルの密度を使用するかどうか\\
\verb| USE_NONHYDRO_DENS_BOUNDARY = .false.,|                        & ; 境界値に静力を満たさない密度を使用するかどうか\\
...
\nmitem{BASENAME_BOUNDARY}は、変換された境界値ファイルのベース名である。
...
``\verb|LINEAR|''と ``\verb|DIST-WEIGHT|''が選択可能である。
...
以上の設定は、\nmitem{BASENAME_BOUNDARY} を除き、\verb|ATMOS|と\verb|OCEAN|もしくは\verb|LAND|の間で共有することができる。
つまり、\namelist{PARAM_MKINIT_REAL_(OCEAN|LAND)}でネームリストの項目を指定しなければ、
\namelist{PARAM_MKINIT_REAL_ATMOS}で設定された値が使用される。
\\
...
%
(2)の場合には、以下の例のように、土壌空隙率($V_v / V$)に対する水が占める割合(飽和度)を
...
\nmitem{upper_qv_type}='\verb|COPY|'の場合、湿度の入力データが存在する最上層のRHを、データが存在しない上層にコピーする。
デフォルトの設定は'\verb|ZERO|'である。
\editboxtwo{
\verb|&PARAM_MKINIT_REAL_GrADS|  & \\
\verb| upper_qv_type = "ZERO"|   & ; 上層でのQVの与え方\\
                                 & ~~~("\verb|ZERO|", "\verb|COPY|")\\
\verb|/|\\
}
