...
時間積分・空間差分スキームに対する他のオプションを表\ref{tab:nml_atm_dyn}に示す。
...
超粘性の微分の階数は\nmitem{ATMOS_DYN_NUMERICAL_DIFF_ORDER}の4倍である。
...
\nmitem{ATMOS_DYN_NUMERICAL_DIFF_COEF}をゼロに設定することができる。
...
\nmitem{ATMOS_DYN_coriolis_type}を「PLANE」に設定する場合は,
