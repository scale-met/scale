...
\begin{tabularx}{150mm}{ll}
 ...
 \verb|ATMOS_DYN_wdamp_height                = 15.D3,|    & \\
\verb|/             | & \\
\end{tabularx}
}}\\
...
力学変数の移流について、SCALEの内部コードにおけるデフォルト値は\nmitem{CD4}となっている。
この時、地形がある場合にはその直上でグリッドスケールに近い鉛直流が発生することがしばしば確認される。
これは\nmitem{UD3}を使用することで緩和される。そのため、実験設定としては\nmitem{UD3}を用いることが推奨される。
\nmitem{UD3}を使う時は、\nmitem{ATMOS_DYN_NUMERICAL_DIFF_COEF}をゼロに設定する。
