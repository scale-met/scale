...
\nmitem{ATMOS_SOLARINS_date}で指定した日時（y,m,d,h,m,s）を用いて、太陽放射を計算するようになる。
このとき、\nmitem{ATMOS_SOLARINS_date}で指定した日時のうち、値が負のものはモデル内の時刻を使う。
例えば、
\nmitem{ATMOS_SOLARINS_date}を\verb|1950,3,21,-1,-1,-1|と指定すると、
計算しているタイムステップのモデル内時刻に応じて、
太陽放射は1950年3月21日（春分）の日の日変化を設定することが可能となる。
