...
ユーザが準備するプロファイルの場合は、任意の層数の高度 [m]、気圧 [Pa]、気温 [K]、水蒸気量 [kg/kg]、オゾン濃度 [kg/kg]
...
\verb| ATMOS_PHY_RD_PROFILE_USER_IN_FILENAME      = ""        | & 気候値を利用しない場合に用いるユーザ指定ファイル (ASCII format)\\
...
表\ref{tab:var_list_atm_rd_offline} にある変数を与える必要がある。
\begin{table}[h]
\begin{center}
  ...
  \begin{tabularx}{150mm}{lXX} \hline
    \rowcolor[gray]{0.9}  変数名 & 変数の説明 & 次元数 \\ \hline
      \verb|SFLX_LW_up|      & 地表面上向き長波放射    & 2次元 \\
      \verb|SFLX_LW_dn_dir|  & 地表面下向き直達長波放射 & 2次元 \\
      \verb|SFLX_LW_dn_diff| & 地表面下向き散乱長波放射 & 2次元 \\
      \verb|SFLX_SW_up|      & 地表面上向き短波放射    & 2次元 \\
      \verb|SFLX_SW_dn_dir|  & 地表面下向き直達短波放射 & 2次元 \\
      \verb|SFLX_SW_dn_diff| & 地表面下向き散乱短波放射 & 2次元 \\
      \verb|RFLX_LW_up|      & 上向き長波放射フラックス & 3次元 \\
      \verb|RFLX_LW_dn|      & 下向き長波放射フラックス & 3次元 \\
      \verb|RFLX_SW_up|      & 上向き短波放射フラックス & 3次元 \\
      \verb|RFLX_SW_dn|      & 下向き短波放射フラックス & 3次元 \\
    \hline
  \end{tabularx}
\end{center}
\end{table}
...
\verb| ATMOS_PHY_RD_OFFLINE_BASENAME              = "radiation", | & ; ファイル名 \\
\verb| ATMOS_PHY_RD_OFFLINE_AXISTYPE              = "XYZ",       | & ; データの次元名 \\
\verb| ATMOS_PHY_RD_OFFLINE_ENABLE_PERIODIC_YEAR  = .false.,     | & ; 年毎の繰返しデータかどうか \\
\verb| ATMOS_PHY_RD_OFFLINE_ENABLE_PERIODIC_MONTH = .false.,     | & ; 月毎の繰返しデータかどうか \\
\verb| ATMOS_PHY_RD_OFFLINE_ENABLE_PERIODIC_DAY   = .false.,     | & ; 日毎の繰返しデータかどうか \\
\verb| ATMOS_PHY_RD_OFFLINE_STEP_FIXED            = -1,          | & ; とある決った時間ステップのデータのみを使う場合にそのステップ番号を指定する \\
\verb| ATMOS_PHY_RD_OFFLINE_CHECK_COORDINATES     = .false,      | & ; ファイル内軸変数の値の確認を行うかどうか \\
\verb| ATMOS_PHY_RD_OFFLINE_STEP_LIMIT            = 100,         | & ; 読み出すデータ数の上限 \\
\verb|/|            & \\
\end{tabularx}
}}}\\
...
\begin{tabularx}{150mm}{lX}
 ...
 \verb| OCEAN_PHY_FILE_basename   = "../init/output/ocean_d01", | & ; 入力ファイル\\
 ...
 \verb| OCEAN_PHY_FILE_step_fixed =  -1,                        | & ; とある決った時間ステップのデータのみを使う場合にそのステップ番号を指定する \\
 \verb| OCEAN_PHY_FILE_CHECK_COORDINATES     = .false,          | & ; ファイル内軸変数の値の確認を行うかどうか \\
 \verb| OCEAN_PHY_FILE_step_limit = 1800,                       | & ; 読み出すデータ数の上限 \\
 \verb|/                                         | & \\
\end{tabularx}
}}\\
