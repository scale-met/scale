...
\verb| ATMOS_PHY_RD_OFFLINE_STEP_FIXED            = 0,           | & ; とある決った時間ステップのデータのみを使う場合にそのステップ番号を指定する。時間変化するデータを与える場合は 0 以下を指定する。\\
\verb| ATMOS_PHY_RD_OFFLINE_CHECK_COORDINATES     = .false,      | & ; ファイル内座標変数の値の確認を行うかどうか \\
...
\begin{tabularx}{150mm}{lX}
 ...
 \verb| OCEAN_PHY_FILE_step_fixed =  -1,                        | & ; とある決った時間ステップのデータのみを使う場合にそのステップ番号を指定する。時間変化するデータを与える場合は 0 以下を指定する。\\
 \verb| OCEAN_PHY_FILE_CHECK_COORDINATES     = .false,          | & ; ファイル内座標変数の値の確認を行うかどうか \\
