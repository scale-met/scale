...
\nmitem{OCEAN_DYN_TYPE}が\verb|"NONE"|や\verb|"OFF"|に設定された場合は、
...
\nmitem{OCEAN_DYN_TYPE}を\verb|"OFFLINE"|に設定した場合には, 海洋表層の力学過程や物理過程は計算されない。
...
(詳細は第\ref{sec:basic_usel_surface}節を参照)。

%-------------------------------------------------------------------------------
\subsubsection{海氷過程}
%-------------------------------------------------------------------------------

\nmitem{OCEAN_ICE_TYPE}を\verb|"SIMPLE"|とした場合、海氷過程が考慮されるようになる。
...
このスキームの設定は、init.confとrun.conf中の\namelist{PARAM_LAND}の\nmitem{LAND_SFC_TYPE}や\nmitem{LAND_SFC_TYPE}で行う。
...
\nmitem{LAND_DYN_BUCKET_nudging_tau}が0であるときは,
海面温度の値は外部ファイルによって完全に置き換わる。
...
\nmitem{LAND_DYN_TYPE}に対して\verb|"NONE"|、\verb|"OFF"|以外が指定されたときには、
...
\nmitem{URBAN_DYN_TYPE}として\verb|LAND|を選択したときには、
