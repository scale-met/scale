...
\begin{enumerate}[1)]
\item ソースファイル内のデフォルト値が、全ての格子に適用される。
...
\nmitem{URBAN_DYN_KUSAKA01_PARAM_IN_FILENAME} の設定例は、\\
\verb|scale/scale-rm/test/data/urban/param.kusaka01.dat| に用意されている。
...
Z0MとAHLの作成方法は基本的にAHと同じである。
ただし、AHとAHLデータは、1時から24時(UTC)までの24時間分のデータを１時間毎に用意する必要があるのに対し、
Z0Mは時間変化を考慮しないので、データは１時刻分でよい (つまり、\nmitem{CNVUSER_NSTEPS}=1)。
``ctl''の\verb|tdef|は \scale では \namelist{PARAM_CNVUSER}の\nmitem{CNVUSER_NSTEPS}に相当する。
