...
\namelist{PARAM_ATMOS} において \nmitem{ATMOS_PHY_RD_TYPE} を \verb|OFFLINE| とした場合には、
データのファイル名やその情報は
...
\namelist{PARAM_MAPPROJECTION}の説明は、第\ref{subsec:adv_mapproj}節を参照されたい。
%
...
上記の\verb|MSTRNX|パラメータファイルは、version 5.2 リリース時に更新された。
...
放射計算で考慮される気体は、水蒸気($H_{2}O$)、二酸化炭素($CO_{2}$)、オゾン($O_{3}$)、一酸化二窒素($N_{2}O$)、一酸化炭素($CO$)、メタン($CH_{4}$)、酸素($O_{2}$)、クロロフルオロカーボン類(CFCs)である。
...
ただし、放射計算で想定される最上層の高度が入力した分布の高度より高い場合は、
外挿が行われることに注意が必要である。\\
