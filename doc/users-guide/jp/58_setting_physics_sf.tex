...
\nmitem{ATMOS_PHY_SF_FLG_MOM_FLUX} が 0 の場合は、運動量に対するバルク交換係数として固定値が使われる。
係数の値は \nmitem{ATMOS_PHY_SF_Const_Cm} で設定する。
\nmitem{ATMOS_PHY_SF_FLG_MOM_FLUX} が 1 の場合は、固定値の摩擦速度を用いて交換係数が計算される。
...
表面温度、粗度長、短波および長波に対するアルベドは、\verb|run.conf|の\namelist{PARAM_ATMOS_PHY_SF_VARS} で設定することができる。
%
\editboxtwo{
\verb|&PARAM_ATMOS_PHY_SF_VARS                      | & \\
\verb| ATMOS_PHY_SF_DEFAULT_SFC_TEMP      = 300.0,  | & ; 地表面温度 \\
\verb| ATMOS_PHY_SF_DEFAULT_SFC_albedo_LW = 0.04,   | & ; 長波放射に対するアルベド \\
\verb| ATMOS_PHY_SF_DEFAULT_SFC_albedo_SW = 0.10,   | & ; 短波放射に対するアルベド \\
\verb| ATMOS_PHY_SF_DEFAULT_SFC_Z0M       = 1E-4,   | & ; 運動量粗度   \\
\verb| ATMOS_PHY_SF_DEFAULT_SFC_Z0H       = 1E-5,   | & ; 熱の粗度     \\
\verb| ATMOS_PHY_SF_DEFAULT_SFC_Z0E       = 1E-5,   | & ; 水蒸気の粗度 \\
\verb|/                                             | & \\
}
