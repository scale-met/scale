...
その場合は、\nmitem{ATMOS_PHY_TB_TYPE}に\verb|SMAGORINSKY|を指定した上で、
以下のように\namelist{PARAM_ATMOS_PHY_TB_SMG}内の\nmitem{ATMOS_PHY_TB_SMG_horizontal}を\verb|.true.|にする。
\editboxtwo{
\verb|&PARAM_ATMOS_PHY_TB_SMG  | \\
\verb| ATMOS_PHY_TB_SMG_horizontal = .true., | \\
\verb|/             | \\
}
何も設定しない場合(default設定)は、\nmitem{ATMOS_PHY_TB_SMG_horizontal}\verb|= .false.|である。
