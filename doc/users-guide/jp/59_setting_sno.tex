...
\nmitem{basename_out}は、出力形式が \scalenetcdf のファイルのベース名である。
\nmitem{nprocs_x_out} と \nmitem{nprocs_y_out} は、出力ファイルの分割数を表す。
デフォルト値は 1 であり、これは複数のファイルが単一のファイルに結合されることを意味する。
SNO に与える MPI プロセス数は、出力ファイルの総数(= \nmitem{nprocs_x_out} x \nmitem{nprocs_y_out})と同じか、
それ以下でなければならないことに注意が必要である。
ただし、\nmitem{output_single}  = \verb|.true.| を設定した場合には、
\nmitem{nprocs_x_out} と \nmitem{nprocs_y_out} の設定は無効となり、
MPIプロセス数によらず、単一のファイルに結合される。


\nmitem{output_grads} を\verb|.true.|にした場合は、\grads で読み込み可能なバイナリ形式で出力される。
\nmitem{dirpath_out}が空であれば、出力先のパスはカレントディレクトリに設定される。
このとき、実行プロセス数は１である必要がある。
出力ファイル名は各変数の名前と同じであり、\nmitem{basename_out}は無視される。
...
また、現時点で、この出力方法は周期境界を設定された実験でのhistoryファイルには対応していないことに注意すること。
...
これを満たすためには、\nmitem{nprocs_x_out}は、入力ファイルの領域内{\XDIR} の格子数(\verb|IMAX| $\times$ \verb|PRC_NUM_X|)の
約数でなければならない(第\ref{sec:domain}章参照)。\nmitem{nprocs_y_out}も同様である。
また、実行時に使用できるMPIプロセス数は変換したいプロセス数を設定すること。

上記の例において、入力ファイル数は 4 ([x,y]=[2,2])、
各ファイルには x 方向と y 方向にそれぞれ(ハロを除いて)30個の格子点が含まれるとする。
つまり、ドメイン全体の格子数は 60 x 60 である。
%
