...
SNO に与える MPI プロセス数は、出力ファイルの総数が 1 より大きい場合には、$\nmitemeq{nprocs_x_out} \times \nmitemeq{nprocs_y_out}$ の約数でなければならない。
出力ファイルの総数が 1 の場合には、プロセス数は入力ファイル総数以下でなければならない。
\proofcomment{要確認}
...
ただし、この例では MPIプロセスの数を 24 ($=4\times6$) まで取ることができる。
...
\subsubsection{複数の \scalenetcdf ファイルから異なる分割数を持つ複数の \Netcdf ファイルへの変換}
%
\editbox{
\verb|&PARAM_SNO                            | \\
\verb| basename_in  = 'input/history_d02',  | \\
...
\proofcomment{basename\_out を設定したときの挙動についてわかりませんでした。dirpath\_out が無視されるということかと推測しました。複数の変数を出力する場合にどうなるのでしょうか？}
...
\end{eqnarray}
\proofcomment{ (lon\_end - lon\_start)/dlon +1 では？}
\noindent
