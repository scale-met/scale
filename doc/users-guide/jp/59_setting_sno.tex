...
この例はMPIプロセスを24個用いて変換するが、出力されるhistoryファイルは1つとなる。
この例では、入力ファイルは単一ファイルに結合され、全ての変数は維持される。
GrADS用のctlファイルを記述すれば、GrADSで読み込むことが可能になる。

現時点で、この出力方法は周期境界を設定された実験でのhistoryファイルには対応していないことに注意すること。
...
\verb| output_single   = .false., | & ; 複数プロセス実行時、単一の\netcdf ファイルに出力するか?\\
