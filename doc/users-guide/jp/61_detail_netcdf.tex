...
\scalelib では、データファイルフォーマットとして{\netcdf}(network Common Data Format)を採用している。
\Netcdf は Unidata (\url{http://www.unidata.ucar.edu}) が開発を行っているソフトウェアであり、自己記述的かつ計算機に依存しないデータ形式を伴うファイルを生成することを可能にする。
例えば、前者についてはファイル中に軸変数と一緒に変数を記述できる長所がある。
また、後者については、どのエンディアンが使用されるかを気にせずにデータを扱える長所がある。
上記の利点に基づいて、ある規約(\scalenetcdf convention )を\scalelib では定めている。
これは、おおよそ CF convection (\url{http://cfconventions.org})に従っている。
...
格子の面積データや体積データについても、それぞれ「cell\_area**」と「cell\_volume**」としてファイルに含まれる。
...
ファイルの中には、地表面高度データ「topo」や、
陸に対するマスクのデータ「lsmask」も含まれる。
...
単一ファイルの入出力機能を用いる場合は複数の時間間隔や圧縮が制限されることに注意されたい。
