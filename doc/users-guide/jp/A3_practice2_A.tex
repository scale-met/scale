...
MPI並列数も変更している場合には、\verb|&PARAM_ATMOS_GRID_CARTESC|の設定において、下記の関係を満たしていれば正解である。
\begin{eqnarray}
...
\nmitem{TIME_DT_ATMOS_DYN} と\nmitem{ATMOS_DYN_TINTEG_SHORT_TYPE} の調整も必要である（第\ref{sec:timeintiv}節参照）。
緩和領域も、格子間隔の20倍から40倍となるように設定しなければならない。
...
ヒストリ変数の一覧は、リファレンスマニュアル(第\ref{sec:reference_manual}節を参照)で確認できる。 \\
...
\nmitem{TIME_DT_ATMOS_RESTART}、\nmitem{TIME_DT_OCEAN_RESTART}、\\
\nmitem{TIME_DT_LAND_RESTART}、\nmitem{TIME_DT_URBAN_RESTART}の値は、3時間(10800秒)の約数かつ\nmitem{TIME_DT}の倍数を指定する必要がある。
