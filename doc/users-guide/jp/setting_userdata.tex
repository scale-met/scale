...
出力ファイル中のタイトル、変数名、変数の説明、変数の単位は、\nmitem{CNVUSER_OUT_TITLE}, \\
\nmitem{CNVUSER_OUT_VARNAME}, \nmitem{CNVUSER_OUT_VARDESC}, \nmitem{CNVUSER_OUT_VARUNIT} でそれぞれ設定する。
ユーザ定義データの種類が \verb|GrADS| の場合は, \nmitem{CNVUSER_OUT_VARNAME} が設定されていなければ \nmitem{CNVUSER_GrADS_VARNAME} が出力変数名として用いられる。
...
出力ファイル中の時刻座標の初期値は \namelist{PARAM_TIME} の \nmitem{TIME_STARTDATE} (\ref{sec:timeintiv}節参照) で指定し、時間間隔は \nmitem{CNVUSER_OUT_DT} で秒単位で指定する。
