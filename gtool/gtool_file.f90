...
contains
  ...
  subroutine FileGetCommoninfoFName( &
    ...
    integer,                    intent(out) :: rankidx     (2)      ! my rank in 2D process topology (x:y)
    integer,                    intent(out) :: procsize    (2)      ! total process size             (x:y)
    integer,                    intent(out) :: size_global (2)      ! total grid size     in global  (x:y)
    integer,                    intent(out) :: start_global(2)      ! start index of grid in global  (x:y)
    integer,                    intent(out) :: halo_global (2,2)    ! halo  grid size     in global  (x:y,start:end)
    integer,                    intent(out) :: halo_local  (2,2)    ! halo  grid size     in local   (x:y,start:end)
    logical,                    intent(out) :: periodic    (2)      ! periodic condition?            (x:y)
  ...
  subroutine FileGetCommoninfoFId( &
    ...
    character(len=File_HMID),   intent(out) :: source               ! for file header
    character(len=File_HMID),   intent(out) :: institution          ! for file header
    integer,                    intent(out) :: myrank      (1)      ! total process size             (x:y)
    integer,                    intent(out) :: rankidx     (2)      ! my rank in 2D process topology (x:y)
    integer,                    intent(out) :: procsize    (2)      ! total process size             (x:y)
    integer,                    intent(out) :: size_global (2)      ! total grid size     in global  (x:y)
    integer,                    intent(out) :: start_global(2)      ! start index of grid in global  (x:y)
    ...
    call FileGetGlobalAttribute( fid, 'myrank',      myrank  (:) )
    call FileGetGlobalAttribute( fid, 'rankidx',     rankidx (:) )
    call FileGetGlobalAttribute( fid, 'procsize',    procsize(:) )

    call FileGetAttribute( fid, 'x', 'size_global',  size_global (1:1) )
    call FileGetAttribute( fid, 'x', 'start_global', start_global(1:1) )
    call FileGetAttribute( fid, 'x', 'halo_global',  halo_global (1,:) )
    call FileGetAttribute( fid, 'x', 'halo_local',   halo_local  (1,:) )
    call FileGetAttribute( fid, 'x', 'periodic',     logical_str       )
    periodic(1) = logical_str == 'true'

    call FileGetAttribute( fid, 'y', 'size_global',  size_global (2:2) )
    call FileGetAttribute( fid, 'y', 'start_global', start_global(2:2) )
    call FileGetAttribute( fid, 'y', 'halo_global',  halo_global (2,:) )
    call FileGetAttribute( fid, 'y', 'halo_local',   halo_local  (2,:) )
    call FileGetAttribute( fid, 'y', 'periodic',     logical_str       )
    periodic(2) = logical_str == 'true'
