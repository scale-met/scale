  ...
  subroutine LATLON_setup( output_dirname )
    ...
    do l = 1, ADM_lall
       ...
       if ( debug ) then
          ...
          call IO_get_fname(fname, trim(output_dirname)//'/checkmap', rank=rgnid, ext='grd', len=8)
...
!       call PRC_abort
    ...
    do l = 1, ADM_lall
       ...
       call IO_get_fname(fname, trim(output_dirname)//'/llmap', rank=rgnid, ext='rgn', len=8)
...
!       rgnid = PRC_RGN_l2r(l)
!
!       call IO_get_fname(fname, trim(output_dirname)//'/llmap', rank=rgnid, ext='rgntxt', len=8)
!
!       fid = IO_get_available_fid()
!       open( unit   = fid,           &
!             file   = trim(fname),   &
