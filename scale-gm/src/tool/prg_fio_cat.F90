...
program fio_cat
  ...
  integer                :: KALL
  integer                :: LALL
  real(SP), allocatable, target :: data4_1D(:)
  real(DP), allocatable, target :: data8_1D(:)
  ...
  if ( use_mpi ) then
     ...
     open(IO_FID_LOG, file='LOG_cat.pe'//trim(rankstr) )
  ...
  do p = pstr, pend
     ...
     do f = 2, fmax
        ...
        ierr = fio_get_pkginfo(hinfo,ifid)
        nmax_data = hinfo%num_of_data
        write(IO_FID_LOG,*) '++input', f, ' : ', trim(infname), "(n=", nmax_data, ")"
