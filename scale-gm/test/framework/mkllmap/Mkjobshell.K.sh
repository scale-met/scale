...
if ls ${TOPDIR}/scale-gm/test/data/grid/boundary/${dir2d}/boundary_${res2d}.pe* > /dev/null 2>&1
then
   echo "#PJM --stgin  \"rank=* ${TOPDIR}/scale-gm/test/data/grid/boundary/${dir2d}/boundary_${res2d}.pe%06r %r:./\"" >> run.sh
elif ls ../../mkhgrid/${dir2d}/boundary_${res2d}.pe* > /dev/null 2>&1
then
   echo "#PJM --stgin  \"rank=* ../../mkhgrid/${dir2d}/boundary_${res2d}.pe%06r %r:./\"" >> run.sh
else
   echo "boundary file is not found!"
   exit 1
fi
...
# run
${MPIEXEC} ./${BINNAME} mkllmap.cnf || exit
