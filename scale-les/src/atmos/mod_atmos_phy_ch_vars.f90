...
contains
  ...
  subroutine ATMOS_PHY_CH_vars_restart_write
    ...
    if ( ATMOS_PHY_CH_RESTART_OUT_BASENAME /= '' ) then
       ...
       write(bname,'(A,A,A)') trim(ATMOS_PHY_CH_RESTART_OUT_BASENAME), '_', trim(timelabel)

       call FILEIO_write( ATMOS_PHY_CH_O3(:,:,:), bname,                ATMOS_PHY_CH_RESTART_OUT_TITLE,        & ! [IN]
                          VAR_NAME(1), VAR_DESC(1), VAR_UNIT(1), 'ZXY', ATMOS_PHY_CH_RESTART_OUT_DTYPE, .true. ) ! [IN]
