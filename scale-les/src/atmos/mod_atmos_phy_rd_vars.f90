...
contains
  ...
  subroutine ATMOS_PHY_RD_vars_restart_write
    ...
    if ( ATMOS_PHY_RD_RESTART_OUT_BASENAME /= '' ) then
       ...
       write(bname,'(A,A,A)') trim(ATMOS_PHY_RD_RESTART_OUT_BASENAME), '_', trim(timelabel)

       call FILEIO_write( ATMOS_PHY_RD_SFLX_LW_dn(:,:), bname,         ATMOS_PHY_RD_RESTART_OUT_TITLE,        & ! [IN]
                          VAR_NAME(1), VAR_DESC(1), VAR_UNIT(1), 'XY', ATMOS_PHY_RD_RESTART_OUT_DTYPE, .true. ) ! [IN]
       call FILEIO_write( ATMOS_PHY_RD_SFLX_SW_dn(:,:), bname,         ATMOS_PHY_RD_RESTART_OUT_TITLE,        & ! [IN]
                          VAR_NAME(2), VAR_DESC(2), VAR_UNIT(2), 'XY', ATMOS_PHY_RD_RESTART_OUT_DTYPE, .true. ) ! [IN]
