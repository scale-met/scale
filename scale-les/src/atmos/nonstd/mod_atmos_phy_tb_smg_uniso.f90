...
!  real(8), private,      save :: TB_length(KA,IA,JA) ! mixing length
  real(8), private,      save :: TB_length_x(KA,IA,JA) ! mixing length
  real(8), private,      save :: TB_length_y(KA,IA,JA) ! mixing length
  real(8), private,      save :: TB_length_z(KA,IA,JA) ! mixing length
  ...
  real(8), private, parameter :: F1ov3      = 1.D0 / 3.D0
...
!    enddo
  ...
  subroutine ATMOS_PHY_TB
    ...
    real(8) :: nu_x(KA,IA,JA)       ! eddy viscosity
    real(8) :: nu_y(KA,IA,JA)       ! eddy viscosity
    real(8) :: nu_z(KA,IA,JA)       ! eddy viscosity
    real(8) :: nuc_x(KA,IA,JA)       ! eddy viscosity
    real(8) :: nuc_y(KA,IA,JA)       ! eddy viscosity
    real(8) :: nuc_z(KA,IA,JA)       ! eddy viscosity
...
!             nu(k,i,j) = ( Cs * TB_length(k,i,j) )**2 * sqrt( temp(k) )
             nu_z(k,i,j) = ( Cs * TB_length_z(k,i,j) )**2 * sqrt( temp(k) )
             nu_x(k,i,j) = ( Cs * TB_length_x(k,i,j) )**2 * sqrt( temp(k) )
             nu_y(k,i,j) = ( Cs * TB_length_y(k,i,j) )**2 * sqrt( temp(k) )
          enddo
       enddo
       enddo
!OCL XFILL
...
!          nu( 1:KS-1,i,j) = 0.D0
!          nu(KE:KA  ,i,j) = 0.D0
          nu_z( 1:KS-1,i,j) = 0.D0
          nu_z(KE:KA  ,i,j) = 0.D0
          nu_x( 1:KS-1,i,j) = 0.D0
          nu_x(KE:KA  ,i,j) = 0.D0
          nu_y( 1:KS-1,i,j) = 0.D0
          nu_y(KE:KA  ,i,j) = 0.D0
       enddo
       enddo
...
!    call COMM_vars8( nu(:,:,:), 1 )
    call COMM_vars8( nu_z(:,:,:), 1 )
    call COMM_vars8( nu_x(:,:,:), 2 )
    call COMM_vars8( nu_y(:,:,:), 3 )
    call COMM_vars8( Pr(:,:,:), 4 )
!    call COMM_wait ( nu(:,:,:), 1 )
    call COMM_wait ( nu_z(:,:,:), 1 )
    call COMM_wait ( nu_x(:,:,:), 2 )
    call COMM_wait ( nu_y(:,:,:), 3 )
    call COMM_wait ( Pr(:,:,:), 4 )
    ...
    IIE = IIS+IBLOCK-1
       ...
       do i = IIS, IIE+1
       do k = KS, KE
          nuc_z(k,i,j) = 0.125D0 * ( nu_z(k  ,i  ,j  ) + nu_z(k-1,i-1,j-1) &
                                 + nu_z(k-1,i  ,j  ) + nu_z(k-1,i-1,j  ) &
                                 + nu_z(k  ,i-1,j  ) + nu_z(k  ,i-1,j-1) &
                                 + nu_z(k  ,i  ,j-1) + nu_z(k-1,i  ,j-1) )
          nuc_x(k,i,j) = 0.125D0 * ( nu_x(k  ,i  ,j  ) + nu_x(k-1,i-1,j-1) &
                                 + nu_x(k-1,i  ,j  ) + nu_x(k-1,i-1,j  ) &
                                 + nu_x(k  ,i-1,j  ) + nu_x(k  ,i-1,j-1) &
                                 + nu_x(k  ,i  ,j-1) + nu_x(k-1,i  ,j-1) )
          nuc_y(k,i,j) = 0.125D0 * ( nu_y(k  ,i  ,j  ) + nu_y(k-1,i-1,j-1) &
                                 + nu_y(k-1,i  ,j  ) + nu_y(k-1,i-1,j  ) &
                                 + nu_y(k  ,i-1,j  ) + nu_y(k  ,i-1,j-1) &
       ...
       do k = KS, KE-1
          qflx_sgs(k,i,j,XDIR) = 0.25D0 * ( DENS(k,i,j)+DENS(k,i+1,j)+DENS(k+1,i,j)+DENS(k+1,i+1,j) ) &
                               * ( -( nu_x(k,i,j)+nu_x(k,i,j-1) ) * Sij_zx(k,i,j) )
       enddo
       enddo
       enddo
       ! at (x, v, interface)
       ...
       do k = KS, KE-1
          qflx_sgs(k,i,j,YDIR) = 0.25D0 * ( DENS(k,i,j)+DENS(k,i,j+1)+DENS(k+1,i,j)+DENS(k+1,i,j+1) ) &
                               * ( -( nu_y(k,i,j)+nu_y(k,i-1,j) ) * Sij_zy(k,i,j) )
       enddo
       enddo
       enddo
       ...
       do k = KS, KE-1
          qflx_sgs(k,i,j,ZDIR) = 0.25D0 * ( DENS(k,i,j)+DENS(k,i+1,j)+DENS(k+1,i,j)+DENS(k+1,i+1,j) ) &
                               * ( -( nu_z(k,i,j)+nu_z(k,i,j-1) ) * Sij_zx(k,i,j) )
       enddo
       enddo
       enddo
       do j = JJS, JJE
       ...
       do k = KS, KE
          qflx_sgs(k,i,j,YDIR) = 0.25D0 * ( DENS(k,i,j)+DENS(k,i+1,j)+DENS(k,i,j+1)+DENS(k,i+1,j+1) ) &
                               * ( -( nu_y(k,i,j)+nu_y(k-1,i,j) ) * Sij_xy(k,i,j) )
       enddo
       enddo
       enddo
       ...
       do k = KS, KE-1
          qflx_sgs(k,i,j,ZDIR) = 0.25D0 * ( DENS(k,i,j)+DENS(k,i,j+1)+DENS(k+1,i,j)+DENS(k+1,i,j+1) ) &
                               * ( -( nu_z(k,i,j)+nu_z(k,i-1,j) ) * Sij_zy(k,i,j) )
       enddo
       enddo
       enddo
       do j = JJS, JJE
       ...
       do k = KS, KE
          qflx_sgs(k,i,j,XDIR) = 0.25D0 * ( DENS(k,i,j)+DENS(k,i+1,j)+DENS(k,i,j+1)+DENS(k,i+1,j+1) ) &
                               * ( -( nu_x(k,i,j)+nu_x(k-1,i,j) ) * Sij_xy(k,i,j) )
       enddo
       enddo
       enddo
       ...
       do k = KS, KE-1
          qflx_sgs(k,i,j,ZDIR) = 0.125D0 * ( DENS(k,i,j)+DENS(k+1,i,j) ) &
                               * ( - ( nu_z(k,i,j) + nu_z(k,i-1,j) + nu_z(k,i,j-1) + nu_z(k,i-1,j-1) ) &
                                   / ( Pr(k,i,j) )                                             &
       ...
       do k = KS, KE
          qflx_sgs(k,i,j,XDIR) = 0.5D0 * ( DENS(k,i,j)+DENS(k,i+1,j) ) &
                               * ( - ( nu_x(k,i,j) + nu_x(k,i,j-1) + nu_x(k-1,i,j) + nu_x(k-1,i,j-1) ) &
       ...
       do k = KS, KE
          qflx_sgs(k,i,j,YDIR) = 0.5D0 * ( DENS(k,i,j)+DENS(k,i,j+1) ) &
                               * ( - ( nu_y(k,i,j) + nu_y(k,i-1,j) + nu_y(k-1,i,j) + nu_y(k-1,i-1,j) ) &
...
!    call HIST_in( nuc (:,:,:), 'NU',   'eddy viscosity',           'm2/s',  '3D', dttb )
    call HIST_in( nuc_x (:,:,:), 'NUX',   'eddy viscosity(x)',           'm2/s',  '3D', dttb )
    call HIST_in( nuc_y (:,:,:), 'NUY',   'eddy viscosity(y)',           'm2/s',  '3D', dttb )
    call HIST_in( nuc_z (:,:,:), 'NUZ',   'eddy viscosity(z)',           'm2/s',  '3D', dttb )
