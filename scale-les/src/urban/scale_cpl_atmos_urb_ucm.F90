...
!!
!! @par History
!! @li      2014-04-18 (S.Adachi)  [new]
!<
!-------------------------------------------------------------------------------
module scale_cpl_atmos_urb_ucm
  !-----------------------------------------------------------------------------
  !
  !++ used modules
  !
  ...
  !++ Public procedure
  !
  public :: CPL_AtmUrb_ucm_setup   ! Ask to Yan-san
  public :: CPL_AtmUrb_ucm         ! Ask to Yan-san
  ...
  integer , private, save :: num_urb_layers = 4
  real(RP), private, save :: DZR(num_urb_layers)     ! thickness of each roof layer [m]
  real(RP), private, save :: DZB(num_urb_layers)     ! thickness of each building layer [m]
  real(RP), private, save :: DZG(num_urb_layers)     ! thickness of each road layer [m]
                                                     ! ( units converted in code to [cm] )
  real(RP), private, save :: ahdiurnal(1:24)         ! AH diurnal profile
  data ahdiurnal=(/0.356,0.274,0.232,0.251,0.375,0.647,0.919,1.135,1.249,1.328,1.365,1.363,1.375,1.404,1.457,1.526,1.557,1.521,1.372,1.206,1.017,0.876,0.684,0.512/)

contains
  !-----------------------------------------------------------------------------
  !
  !-----------------------------------------------------------------------------
  subroutine CPL_AtmUrb_ucm_setup( CPL_TYPE_AtmUrb )        ! Ask to Yansan
    use scale_process, only: &
       PRC_MPIstop

    implicit none

    character(len=H_SHORT), intent(in) :: CPL_TYPE_AtmUrb   ! Ask to Yansan

    real(RP) :: CPL_AtmUrb_ucm_ZR
    real(RP) :: CPL_AtmUrb_ucm_roof_width
    real(RP) :: CPL_AtmUrb_ucm_road_width
    real(RP) :: CPL_AtmUrb_ucm_SIGMA_ZED
    real(RP) :: CPL_AtmUrb_ucm_AH
    real(RP) :: CPL_AtmUrb_ucm_ALH
    real(RP) :: CPL_AtmUrb_ucm_BETR
    real(RP) :: CPL_AtmUrb_ucm_BETB
    real(RP) :: CPL_AtmUrb_ucm_BETG
    real(RP) :: CPL_AtmUrb_ucm_CAPR
    real(RP) :: CPL_AtmUrb_ucm_CAPB
    real(RP) :: CPL_AtmUrb_ucm_CAPG
    real(RP) :: CPL_AtmUrb_ucm_AKSR
    real(RP) :: CPL_AtmUrb_ucm_AKSB
    real(RP) :: CPL_AtmUrb_ucm_AKSG
    real(RP) :: CPL_AtmUrb_ucm_ALBR
    real(RP) :: CPL_AtmUrb_ucm_ALBB
    real(RP) :: CPL_AtmUrb_ucm_ALBG
    real(RP) :: CPL_AtmUrb_ucm_EPSR
    real(RP) :: CPL_AtmUrb_ucm_EPSB
    real(RP) :: CPL_AtmUrb_ucm_EPSG
    real(RP) :: CPL_AtmUrb_ucm_Z0R
    real(RP) :: CPL_AtmUrb_ucm_Z0B
    real(RP) :: CPL_AtmUrb_ucm_Z0G
    real(RP) :: CPL_AtmUrb_ucm_TRLEND
    real(RP) :: CPL_AtmUrb_ucm_TBLEND
    real(RP) :: CPL_AtmUrb_ucm_TGLEND
    integer  :: CPL_AtmUrb_ucm_BOUND

    NAMELIST / PARAM_CPL_ATMURB_UCM / &
       CPL_AtmUrb_ucm_ZR,         &
       CPL_AtmUrb_ucm_roof_width, &
       CPL_AtmUrb_ucm_road_width, &
       CPL_AtmUrb_ucm_SIGMA_ZED,  &
       CPL_AtmUrb_ucm_AH,         &
       CPL_AtmUrb_ucm_ALH,        &
       CPL_AtmUrb_ucm_BETR,       &
       CPL_AtmUrb_ucm_BETB,       &
       ...
       CPL_AtmUrb_ucm_Z0B,        &
       CPL_AtmUrb_ucm_Z0G,        &
       CPL_AtmUrb_ucm_TRLEND,     &
       CPL_AtmUrb_ucm_TBLEND,     &
       CPL_AtmUrb_ucm_TGLEND,     &
       CPL_AtmUrb_ucm_BOUND,      &

    integer :: ierr
    !---------------------------------------------------------------------------

    CPL_AtmUrb_ucm_ZR           = ZR
    CPL_AtmUrb_ucm_roof_width   = roof_width
    CPL_AtmUrb_ucm_road_width   = road_width
    CPL_AtmUrb_ucm_SIGMA_ZED    = SIGMA_ZED
    CPL_AtmUrb_ucm_AH           = AH
    CPL_AtmUrb_ucm_ALH          = ALH
    CPL_AtmUrb_ucm_BETR         = BETR
    CPL_AtmUrb_ucm_BETB         = BETB
    CPL_AtmUrb_ucm_BETG         = BETG
    CPL_AtmUrb_ucm_CAPR         = CAPR
    CPL_AtmUrb_ucm_CAPB         = CAPB
    CPL_AtmUrb_ucm_CAPG         = CAPG
    CPL_AtmUrb_ucm_AKSR         = AKSR
    CPL_AtmUrb_ucm_AKSB         = AKSB
    CPL_AtmUrb_ucm_AKSG         = AKSG
    CPL_AtmUrb_ucm_ALBR         = ALBR
    CPL_AtmUrb_ucm_ALBB         = ALBB
    CPL_AtmUrb_ucm_ALBG         = ALBG
    CPL_AtmUrb_ucm_EPSR         = EPSR
    CPL_AtmUrb_ucm_EPSB         = EPSB
    CPL_AtmUrb_ucm_EPSG         = EPSG
    CPL_AtmUrb_ucm_Z0R          = Z0R
    CPL_AtmUrb_ucm_Z0B          = Z0B
    CPL_AtmUrb_ucm_Z0G          = Z0G
    CPL_AtmUrb_ucm_TRLEND       = TRLEND 
    CPL_AtmUrb_ucm_TBLEND       = TBLEND
    CPL_AtmUrb_ucm_TGLEND       = TGLEND
    CPL_AtmUrb_ucm_BOUND        = BOUND


    if( IO_L ) write(IO_FID_LOG,*)
    if( IO_L ) write(IO_FID_LOG,*) '*** Atmos-Urban: urban canopy model parameter'

    if ( CPL_TYPE_AtmUrb /= 'UCM' ) then         ! Ask to Yansan
    ...
    ZR           = CPL_AtmUrb_ucm_ZR
    roof_width   = CPL_AtmUrb_ucm_roof_width
    road_width   = CPL_AtmUrb_ucm_road_width
    SIGMA_ZED    = CPL_AtmUrb_ucm_SIGMA_ZED
    AH           = CPL_AtmUrb_ucm_AH
    ALH          = CPL_AtmUrb_ucm_ALH
    BETR         = CPL_AtmUrb_ucm_BETR
    BETB         = CPL_AtmUrb_ucm_BETB
    BETG         = CPL_AtmUrb_ucm_BETG
    CAPR         = CPL_AtmUrb_ucm_CAPR
    CAPB         = CPL_AtmUrb_ucm_CAPB
    CAPG         = CPL_AtmUrb_ucm_CAPG
    AKSR         = CPL_AtmUrb_ucm_AKSR
    AKSB         = CPL_AtmUrb_ucm_AKSB
    AKSG         = CPL_AtmUrb_ucm_AKSG
    ALBR         = CPL_AtmUrb_ucm_ALBR
    ALBB         = CPL_AtmUrb_ucm_ALBB
    ALBG         = CPL_AtmUrb_ucm_ALBG
    EPSR         = CPL_AtmUrb_ucm_EPSR
    EPSB         = CPL_AtmUrb_ucm_EPSB
    EPSG         = CPL_AtmUrb_ucm_EPSG
    Z0R          = CPL_AtmUrb_ucm_Z0R
    Z0B          = CPL_AtmUrb_ucm_Z0B
    Z0G          = CPL_AtmUrb_ucm_Z0G
    TRLEND       = CPL_AtmUrb_ucm_TRLEND
    TBLEND       = CPL_AtmUrb_ucm_TBLEND
    TGLEND       = CPL_AtmUrb_ucm_TGLEND
    BOUND        = CPL_AtmUrb_ucm_BOUND
...
!-----------------------------------------------------------------------------
  ...
  subroutine CPL_AtmUrb_ucm( &
        TR_URB, TB_URB, TG_URB, TC_URB, QC_URB, UC_URB,   & ! (inout)
        TRL_URB, TBL_URB, TGL_URB,                & ! (inout)
        SWUFLX_URB, LWUFLX_URB,                   & ! (out)
        SHFLX_URB, LHFLX_URB, GHFLX_URB,          & ! (out)
        TS_URB,                                   & ! (out)
        DZ, DENS, MOMX, MOMY, MOMZ,               & ! (in)
        TEMP, QV, SWD, LWD, PREC                  ) ! (in)
    ...
    real(RP), intent(inout) :: TR_URB(IA,JA) ! Surface temperature of roof [K]
    real(RP), intent(inout) :: TB_URB(IA,JA) ! Surface temperature of building [K]
    real(RP), intent(inout) :: TG_URB(IA,JA) ! Surface temperature of ground [K]
    real(RP), intent(inout) :: TC_URB(IA,JA) ! Diagnostic canopy air temperature [K]
    real(RP), intent(inout) :: QC_URB(IA,JA) ! Diagnostic canopy humidity [-]
    real(RP), intent(inout) :: UC_URB(IA,JA) ! Diagnostic canopy wind [m/s]
    real(RP), intent(inout) :: TRL_URB(IA,JA,1:num_urb_layers) ! temperature in layer of roof [K]
    real(RP), intent(inout) :: TBL_URB(IA,JA,1:num_urb_layers) ! temperature in layer of building [K]
    real(RP), intent(inout) :: TGL_URB(IA,JA,1:num_urb_layers) ! temperature in layer of ground [K]   

    real(RP), intent(out)   :: SWUFLX_URB(IA,JA) ! upward shortwave flux at the surface [W/m2]
    real(RP), intent(out)   :: LWUFLX_URB(IA,JA) ! upward longwave flux at the surface [W/m2]
    real(RP), intent(out)   :: SHFLX_URB (IA,JA) ! sensible heat flux at the surface [W/m2]
    real(RP), intent(out)   :: LHFLX_URB (IA,JA) ! latent heat flux at the surface [W/m2]
    real(RP), intent(out)   :: GHFLX_URB (IA,JA) ! ground heat flux at the surface [W/m2]
    real(RP), intent(out)   :: TS_URB (IA,JA)    ! diagnostic surface temperature [K]
    ...
    real(RP), intent(in)    :: TEMP(IA,JA) ! atmospheric air temperature at 1st atmospheric level [K]  
    ...
    real(RP), intent(in)    :: PREC(IA,JA) ! precipitation [mm/h] ! Please check the unit, before you use.
...
!-----------------------------------------------------------------------------
  subroutine urban(LSOLAR,                            & ! (in)
    ...
    use scale_time, only:       &
      ...
      DELT => TIME_DTSEC_URB        !< time interval of urban step [sec] 

    implicit none

   !-- parameters
   real(RP), private, parameter    :: CP=0.24          ! heat capacity of dry air  [cgs unit]
   real(RP), private, parameter    :: EL=583.          ! latent heat of vaporation [cgs unit]
   real(RP), private, parameter    :: SIG=8.17E-11     ! stefun bolzman constant   [cgs unit]
     
   real(RP), private, parameter    :: SRATIO=0.75      ! ratio between direct/total solar [-]
   ...
   !!   real(RP), intent(in)    :: DECLIN ! solar declination                    [rad]
   !!   real(RP), intent(in)    :: COSZ ! sin(fai)*sin(del)+cos(fai)*cos(del)*cos(omg)
   !!   real(RP), intent(in)    :: OMG  ! solar hour angle                       [rad]
   ...
   logical  :: SHADOW=.false.  
          ! [true=consider svf and shadow effects, false=consider svf effect only]
...
!-----------------------------------------------------------
   ...
   ! tloc=mod(int(OMG/PI*180./15.+12.+0.5 ),24)
     tlon=mod( (int(TIME/(60.0_DP*60.0_DP)) + int(XLON/15.0_DP)),24 )
     if(tloc==0) tloc=24
...
!-----------------------------------------------------------
   ...
   CD = KARMAN**2./(ALOG(Z/Z0)-PSIM)**2.
!
   CH = 0.4**2./(ALOG(Z/Z0)-PSIM)/(ALOG(Z/Z0H)-PSIH)
...
!------------------------------------------------------------
   ...
   subroutine canopy_wind(ZA, UA, UC)
    ...
    real(RP),intent(in)  :: UA   ! wind speed at 1st atmospheric level [m/s]
    real(RP),intent(out) :: UA   ! wind speed at 1st atmospheric level [m/s]
...
end module scale_cpl_atmos_urb_ucm
