...
module mod_atmos_phy_tb_vars
  ...
  public :: ATMOS_PHY_TB_vars_restart_read_var
...
contains
  ...
  subroutine ATMOS_PHY_TB_vars_restart_open
    ...
    if ( ATMOS_PHY_TB_RESTART_IN_BASENAME /= '' ) then
       ...
       call FILEIO_open( restart_fid, ATMOS_PHY_TB_RESTART_IN_BASENAME )
    else
...
!       call STAT_total( total, ATMOS_PHY_TB_NU (:,:,:), VAR_NAME(2) )
  ...
  !-----------------------------------------------------------------------------
  !> Read restart
  subroutine ATMOS_PHY_TB_vars_restart_read_var
    use scale_fileio, only: &
       FILEIO_read_var, &
       FILEIO_flush
    use scale_rm_statistics, only: &
    ...
    if ( restart_fid .NE. -1 ) then
       ...
       call FILEIO_read_var( ATMOS_PHY_TB_TKE(:,:,:),                & ! [OUT]
                             restart_fid, VAR_NAME(1), 'ZXY', step=1 ) ! [IN]
!       call FILEIO_read_var( ATMOS_PHY_TB_NU (:,:,:),                & ! [OUT]
...
!       call STAT_total( total, ATMOS_PHY_TB_NU (:,:,:), VAR_NAME(2) )
    ...
    return
  end subroutine ATMOS_PHY_TB_vars_restart_read_var
