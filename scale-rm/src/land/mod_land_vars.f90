...
contains
  ...
  subroutine LAND_vars_history
    ...
    call FILE_HISTORY_in( LAND_SFC_TEMP  (:,:),      VAR_NAME(I_SFC_TEMP),   VAR_DESC(I_SFC_TEMP),   VAR_UNIT(I_SFC_TEMP), standard_name=VAR_STDN(I_SFC_TEMP) )
  ...
  subroutine LAND_vars_restart_write
    ...
    if ( restart_fid /= -1 ) then
       ...
       call FILE_CARTESC_write_var( restart_fid, VAR_ID(I_SFLX_water), LAND_SFLX_water(:,:),     & ! [IN]
                                    VAR_NAME(I_SFLX_water), 'XY',   fill_halo=.true.             ) ! [IN]
       call FILE_CARTESC_write_var( restart_fid, VAR_ID(I_SFLX_ice), LAND_SFLX_ice(:,:),         & ! [IN]
