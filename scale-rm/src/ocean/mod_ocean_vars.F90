...
module mod_ocean_vars
  ...
  integer,                private, parameter :: VMAX              = 23 !< number of the variables
  ...
  integer,                private, parameter :: I_SFC_Z0M         = 12
  integer,                private, parameter :: I_SFC_Z0H         = 13
  integer,                private, parameter :: I_SFC_Z0E         = 14
  integer,                private, parameter :: I_SFLX_MW         = 15
  integer,                private, parameter :: I_SFLX_MU         = 16
  integer,                private, parameter :: I_SFLX_MV         = 17
  integer,                private, parameter :: I_SFLX_SH         = 18
  integer,                private, parameter :: I_SFLX_LH         = 19
  integer,                private, parameter :: I_SFLX_evap       = 20
  integer,                private, parameter :: I_SFLX_WH         = 21
  integer,                private, parameter :: I_SFLX_water      = 22
  integer,                private, parameter :: I_SFLX_ice        = 23
...
contains
  ...
  subroutine OCEAN_vars_setup
    ...
    if ( OCEAN_RESTART_IN_BASENAME /= '' ) then
       ...
       LOG_INFO("OCEAN_vars_setup",*) 'Add timelabel?  : ', OCEAN_RESTART_IN_POSTFIX_TIMELABEL
    else
    ...
    if (       OCEAN_RESTART_OUTPUT             &
       ...
       LOG_INFO("OCEAN_vars_setup",*) 'Add timelabel?  : ', OCEAN_RESTART_OUT_POSTFIX_TIMELABEL
    else
  ...
  subroutine OCEAN_vars_restart_open
    ...
    if ( OCEAN_do .and. OCEAN_RESTART_IN_BASENAME /= '' ) then
       ...
       LOG_INFO("OCEAN_vars_restart_open",*) 'basename: ', trim(basename)
       ...
       LOG_INFO("OCEAN_vars_restart_open",*) 'restart file for ocean is not specified.'
    endif
...
!                     VAR_NAME(I_VVEL),                                      __FILE__, __LINE__ )
    ...
    call FILE_HISTORY_in( OCEAN_SFC_TEMP  (:,:),                     VAR_NAME(I_SFC_TEMP),                                    &
                          VAR_DESC(I_SFC_TEMP),        VAR_UNIT(I_SFC_TEMP),        standard_name=VAR_STDN(I_SFC_TEMP)        )
    call FILE_HISTORY_in( OCEAN_SFC_albedo(:,:,I_R_direct ,I_R_IR ), VAR_NAME(I_SFC_ALB_IR_dir),                              &
                          VAR_DESC(I_SFC_ALB_IR_dir),  VAR_UNIT(I_SFC_ALB_IR_dir),  standard_name=VAR_STDN(I_SFC_ALB_IR_dir)  )
    call FILE_HISTORY_in( OCEAN_SFC_albedo(:,:,I_R_diffuse,I_R_IR ), VAR_NAME(I_SFC_ALB_IR_dif),                              &
                          VAR_DESC(I_SFC_ALB_IR_dif),  VAR_UNIT(I_SFC_ALB_IR_dif),  standard_name=VAR_STDN(I_SFC_ALB_IR_dif)  )
    call FILE_HISTORY_in( OCEAN_SFC_albedo(:,:,I_R_direct ,I_R_NIR), VAR_NAME(I_SFC_ALB_NIR_dir),                             &
                          VAR_DESC(I_SFC_ALB_NIR_dir), VAR_UNIT(I_SFC_ALB_NIR_dir), standard_name=VAR_STDN(I_SFC_ALB_NIR_dir) )
    call FILE_HISTORY_in( OCEAN_SFC_albedo(:,:,I_R_diffuse,I_R_NIR), VAR_NAME(I_SFC_ALB_NIR_dif),                             &
                          VAR_DESC(I_SFC_ALB_NIR_dif), VAR_UNIT(I_SFC_ALB_NIR_dif), standard_name=VAR_STDN(I_SFC_ALB_NIR_dif) )
    call FILE_HISTORY_in( OCEAN_SFC_albedo(:,:,I_R_direct ,I_R_VIS), VAR_NAME(I_SFC_ALB_VIS_dir),                             &
                          VAR_DESC(I_SFC_ALB_VIS_dir), VAR_UNIT(I_SFC_ALB_VIS_dir), standard_name=VAR_STDN(I_SFC_ALB_VIS_dir) )
    call FILE_HISTORY_in( OCEAN_SFC_albedo(:,:,I_R_diffuse,I_R_VIS), VAR_NAME(I_SFC_ALB_VIS_dif),                             &
                          VAR_DESC(I_SFC_ALB_VIS_dif), VAR_UNIT(I_SFC_ALB_VIS_dif), standard_name=VAR_STDN(I_SFC_ALB_VIS_dif) )
    call FILE_HISTORY_in( OCEAN_SFC_Z0M   (:,:),                     VAR_NAME(I_SFC_Z0M),                                     &
                          VAR_DESC(I_SFC_Z0M),         VAR_UNIT(I_SFC_Z0M),         standard_name=VAR_STDN(I_SFC_Z0M)         )
    call FILE_HISTORY_in( OCEAN_SFC_Z0H   (:,:),                     VAR_NAME(I_SFC_Z0H),                                     &
                          VAR_DESC(I_SFC_Z0H),         VAR_UNIT(I_SFC_Z0H),         standard_name=VAR_STDN(I_SFC_Z0H)         )
    call FILE_HISTORY_in( OCEAN_SFC_Z0E   (:,:),                     VAR_NAME(I_SFC_Z0E),                                     &
                          VAR_DESC(I_SFC_Z0E),         VAR_UNIT(I_SFC_Z0E),         standard_name=VAR_STDN(I_SFC_Z0H)         )
  ...
  subroutine OCEAN_vars_restart_create
    ...
    if ( OCEAN_do .and. OCEAN_RESTART_OUT_BASENAME /= '' ) then
       ...
       LOG_INFO("OCEAN_vars_restart_create",*) 'basename: ', trim(basename)
