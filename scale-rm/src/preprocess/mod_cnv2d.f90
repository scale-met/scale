...
contains
  ...
  subroutine CNV2D_setup
    ...
    if( IO_L ) write(IO_FID_LOG,*) '++++++ Module[convert 2d data] / Categ[preprocess] / Origin[SCALE-RM]'
    ...
    if ( CNV2D_UseGrADS ) then
       ...
       if( IO_L ) write(IO_FID_LOG,*) '*** Use GrADS format file for input'
    endif

    if    ( CNV2D_interpolation_type == 'bilinear'        ) then
       if( IO_L ) write(IO_FID_LOG,*) '*** Interpolation type : bi-linear interpolation (coarse input->fine model grid)'
    elseif( CNV2D_interpolation_type == 'areaweighted'    ) then
       if( IO_L ) write(IO_FID_LOG,*) '*** Interpolation type : area-weighted average   (fine input->coarse model grid)'
    ...
    if ( CNV2D_DoNothing ) then
       if( IO_L ) write(IO_FID_LOG,*) '*** Do nothing for 2D data conversion'
    else
       ...
       if( IO_L ) write(IO_FID_LOG,*) '*** The size of tile [deg] = ', CNV2D_unittile_ddeg
       if( IO_L ) write(IO_FID_LOG,*) '*** oversampling factor    = ', CNV2D_oversampling_factor
    endif
  ...
  subroutine CNV2D_GrADS
    ...
    if( IO_L ) write(IO_FID_LOG,*)
    if( IO_L ) write(IO_FID_LOG,*) '++++++ Module[convert 2D GrADS] / Categ[preprocess] / Origin[SCALE-RM]'
    ...
    if( IO_L ) write(IO_FID_LOG,*) '*** Number of steps : ', GrADS_NSTEP
    if ( GrADS_NSTEP > 1 ) then
       call CALENDAR_unit2sec( VAR2D_DTSEC,  GrADS_DT,  GrADS_DT_UNIT  )
       if( IO_L ) write(IO_FID_LOG,*) '*** Time interval   : ', VAR2D_DTSEC
    else
       VAR2D_DTSEC = 0.0_DP
    endif
    ...
    REAL_LONU_mod(:,:) = mod( REAL_LONU(:,:)+3.0_DP*PI, 2.0_DP*PI ) - PI ! [-> 0..180,-180..0]
    ...
    if( IO_L ) write(IO_FID_LOG,*) '*** Size of data in each tile (j) = ', TILE_NLAT
    if( IO_L ) write(IO_FID_LOG,*) '*** Size of data in each tile (i) = ', TILE_NLON
    ...
    do nowstep = 1, GrADS_NSTEP
       ...
       if( IO_L ) write(IO_FID_LOG,*) '*** step = ', nowstep
    ...
    do t = 1, TILE_nmax
       ...
       if ( hit_lat .AND. hit_lon ) then
          ...
          do jj = 1, TILE_NLAT+1
             ...
             !if( IO_L ) write(IO_FID_LOG,*) jj, TILE_LAT(jj) / D2R
          enddo
          ...
          do ii = 1, TILE_NLON+1
             ...
             !if( IO_L ) write(IO_FID_LOG,*) ii, TILE_LON(ii) / D2R
          enddo
...
!             if( IO_L ) write(IO_FID_LOG,*) ii, jj, area, iloc, jloc, ifrac_l, jfrac_b, TILE_VALUE(ii,jj)
  ...
  subroutine CNV2D_write( &
    ...
    if ( CNV2D_OUT_BASENAME /= '' ) then
       if( IO_L ) write(IO_FID_LOG,*)
       if( IO_L ) write(IO_FID_LOG,*) '*** Output converted 2D file ***'
