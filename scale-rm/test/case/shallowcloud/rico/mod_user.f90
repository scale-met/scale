...
module mod_user
  ...
  ! flag
  integer,  private              :: USER_LS_FLG        = 0      ! 0->no force, 1->dycoms, 2->rico
...
contains
  ...
  subroutine USER_step
    ...
    real(RP) :: QHYD, RovCP, PRES, CPovCV
    ...
    if ( MOMZ_LS_FLG(I_QTRC) ) then
       ...
       if ( USER_LS_FLG == 2 ) then ! RICO
          ...
          do k = KS, KE
             ratesum = 0.0_RP
             do iq = 1, QA
                ratesum = ratesum + QTRC(k,i,j,iq)*TRACER_MASS(iq)
             enddo

             do iq = 1, QA
                Q_rate(k,i,j,iq) = QTRC(k,i,j,iq)*TRACER_MASS(iq) / ratesum
             enddo
          enddo
          enddo
          enddo
