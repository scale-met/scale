...
contains
  ...
  subroutine USER_step
    ...
    if ( front_posi > 0.0_RP .and. front_posi < GRID_CARTESC_FX(IA) ) then
       mini = 9.999D10
       do i = 1, IA
          diff = GRID_CARTESC_FX(i) - front_posi
       ...
       dist1 = mini
       dist2 = GRID_CARTESC_FDX(front_grid) - dist1
       fact_A = dist1 / GRID_CARTESC_FDX(front_grid)
       fact_B = dist2 / GRID_CARTESC_FDX(front_grid)
    ...
    elseif ( front_posi > GRID_CARTESC_FX(IE) ) then
          do j = 1, JA
          do i = 1, IA
          do k = 1, KA
             ATMOS_BOUNDARY_var(k,i,j,I_BND_DENS) = DENS_INIT(k) + DENS_tend(k,NUM_RELAX_GRIDS)
