...
~\\
\proofcomment{(八代)「京」と「インテル機」の対比はどういう意味か？CPUアーキテクチャということ？それとも規模？}\\
\replycomment{(足立) 両方かと思います。}\\
\proofcomment{(八代)並列計算機に対してチューニングとは？}\\
\replycomment{(足立)なかなか、ここで、チューニングしてると書いても具体性がないので、少し削除\&修正しました}\\
~\\
