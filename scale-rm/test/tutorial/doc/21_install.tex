...
\begin{itemize}
  ...
  \item {\bf 描画環境}：GrADS\footnote{http://cola.gmu.edu/grads/}、
GPhys/Ruby-DCL\footnote{https://www.gfd-dennou.org/arch/ruby/products/gphys/}、
ncview\footnote{http://meteora.ucsd.edu/~piece/ncview\_home\_page.html}など。
  \item {\bf 演算性能評価}：PAPIライブラリ\footnote{http://icl.utk.edu/papi/}が使用可能。
\end{itemize}
...
よりダウンロードできる。
ソースコードのtarballファイルを展開すると\verb|scale-{ver}/|というディレクトリができる。
\verb|{ver}|には、バーション番号が入る。現行は、``\version''である。
\begin{verbatim}
...
\end{verbatim}
%%% ?????
%%% verbatim環境で、マクロ\versionを表記するには？？？？？
%%% ?????
...
Makdefファイルは、\verb|scale-{ver}/sysdep/|内にいくつかの計算機環境に
...
\begin{verbatim}
 $ cd scale-$ver/scale-rm/test/tutorial/bin
 $ make -j 4
\end{verbatim}
...
下記のコマンドによって生成される。
\begin{verbatim}
 $ cd scale-{ver}/scale-rm/util/netcdf2grads_h
 $ make -j 2
\end{verbatim}
