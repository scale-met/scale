...
\begin{itemize}
  ...
  \item {\bf ファイルI/Oライブラリ} : gzip、HDF5、NetCDF4を必要とする。\\
        HDF5/NetCDF4の代わりに、NetCDF3でも動作するが、この場合はSCALEが提供する全ての機能が有効にならない。
  \end{itemize}
\end{itemize}
...
\begin{tabularx}{150mm}{|l|l|X|} \hline
 ...
 CentOS                & 6.5、6.6、7.0、7.1 & 左記のバージョン以外での動作は保証しない \\ \hline
 openSUSE              & 13.2               &  \\ \hline
 SUSE Enterprise Linux & 11.1、11.3         & 11.1ではGNUコンパイラは使用不可 \\ \hline
 fedora                & 20                 & 左記のバージョン以外での動作は保証しない \\ \hline
...
\begin{tabularx}{150mm}{|l|l|X|} \hline
 ...
 Intel (icc/ifort)     & 13.0.1、14.0.2、15.0.0 & 2013年以降のバージョンであれば左記以外も正常動作する可能性が高い（動作は保証しない）\\ \hline
...
SCALEはコンパイルするとき、独自の環境変数"\verb|SCALE_SYS|"に設定した
...
\verb|NETCDF_LIBS    ?= -L$(NETCDF4)/lib -L$(HDF5)/lib| \\
\verb|　　　-lnetcdff -lnetcdf -lhdf5_hl -lhdf5 -lm -lz| \\
\verb| ～～中略～～| \\
\end{tabularx}
}}\\
