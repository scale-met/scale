...
%\begin{enumerate}
%\item 初期値の作成(mpirun -np *** scale-rm\_init init.conf)
%\item 実験の実行(mpirun -np *** scale-rm run.conf)
%\end{enumerate}
...
%合った初期値・境界値を作成し，その後scale-rmモデル本体を実行する．
...
%\begin{verbatim}
%$ mpirun -n 6 ./scale-rm_init init.conf
%\end{verbatim}
%正常にJOBが終了すれば，
