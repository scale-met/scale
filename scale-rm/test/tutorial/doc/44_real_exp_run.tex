...
コンフィグファイルが準備されており、
ドメインの位置や格子点数など、チュートリアル用の設定（表\ref{tab:grids}）に合わせてある。
モデル本体の実行には
...
\verb|   RESTART_IN_BASENAME = "../init/init_d01_20140810-000000.000",| \\
\verb|/| \\
 \\
...
\verb| TIME_DT            = 60.0D0,| &← トレーサー移流計算の時間ステップ\\
\verb| TIME_DT_UNIT       = "SEC",|  &← \verb|TIME_DT|の単位\\
\verb| TIME_DT_ATMOS_DYN  = 30.0D0,| &← トレーサー移流計算以外の力学過程の時間ステップ\\
