...
側面境界条件の設定は、設定ファイルの\verb|PARAM_PRC|の項目を編集することで設定できる。
\textcolor{red}{\bf この設定は、pp\_***.conf、init\_***.conf、run\_***.confの設定ファイルの間で
必ず一致させなければならない。} 周期境界条件を設定したい場合は、デフォルト設定なので特に設定ファイルに記述する
必要はない。理想実験チュートリアルの設定ファイルの\verb|PARAM_PRC|の項目を見れば特に境界条件に関する記述が
...
\noindent 外部入力データ指定の場合は、\ref{sec:buffer}節で説明したように必ず側面境界の緩和領域を
設定しなければならない。どの変数を強制させるか、またその場合の時定数などの
設定は設定ファイルの\verb|PARAM_ATMOS_BOUNDARY|の項目で指定できる。現実大気実験のチュートリアルの
設定ファイル（\verb|run.conf|）を元にして一部の項目を説明する。\\
...
上から3つ目の設定項目である``\verb|ATMOS_BOUNDARY_USE_VELZ = .true.|''は、上部バッファー層で鉛直速度に対して「強制をかける」ことを意味している。側面のバッファー領域には影響しないことに注意する必要がある。
一方、``\verb|ATMOS_BOUNDARY_USE_QHYD = .false.|''として、凝結物の混合比に対しては「強制をかけない」設定になっている。この設定は側面のバッファー層にも有効である。
その次の項目の``\verb| ATMOS_BOUNDARY_VALUE_VELZ|''は、上部バッファー層で鉛直速度に対して強制をかける際、
ここで指定した値、``0.0 m/s''へ近づくように強制をかけるという指定を意味する。
最後の行の``\verb|ATMOS_BOUNDARY_UPDATE_DT|''は、外部入力データの更新間隔が21600秒であることを
意味している。たとえば、6時間間隔でデータが与えられている再解析データを外部入力データとして使用する場合にこの設定になる。二つの外部入力データ時間の間は、線形内挿される。
