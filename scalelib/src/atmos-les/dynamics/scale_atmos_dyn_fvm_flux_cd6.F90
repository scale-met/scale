...
!<
!-------------------------------------------------------------------------------
! Warning: This file was generated from atmos-les/dynamics/scale_atmos_dyn_fvm_flux_udcd.F90.erb.
...
module scale_atmos_dyn_fvm_flux_cd6
  ...
  !++ Public procedure
  !
  public :: ATMOS_DYN_FVM_flux_valueW_Z_cd6

  public :: ATMOS_DYN_FVM_fluxZ_XYZ_cd6
  public :: ATMOS_DYN_FVM_fluxX_XYZ_cd6
  public :: ATMOS_DYN_FVM_fluxY_XYZ_cd6

  public :: ATMOS_DYN_FVM_fluxZ_XYW_cd6
  public :: ATMOS_DYN_FVM_fluxJ13_XYW_cd6
  public :: ATMOS_DYN_FVM_fluxJ23_XYW_cd6
  public :: ATMOS_DYN_FVM_fluxX_XYW_cd6
  public :: ATMOS_DYN_FVM_fluxY_XYW_cd6

  public :: ATMOS_DYN_FVM_fluxZ_UYZ_cd6
  public :: ATMOS_DYN_FVM_fluxJ13_UYZ_cd6
  public :: ATMOS_DYN_FVM_fluxJ23_UYZ_cd6
  public :: ATMOS_DYN_FVM_fluxX_UYZ_cd6
  public :: ATMOS_DYN_FVM_fluxY_UYZ_cd6

  public :: ATMOS_DYN_FVM_fluxZ_XVZ_cd6
  public :: ATMOS_DYN_FVM_fluxJ13_XVZ_cd6
  public :: ATMOS_DYN_FVM_fluxJ23_XVZ_cd6
  public :: ATMOS_DYN_FVM_fluxX_XVZ_cd6
  public :: ATMOS_DYN_FVM_fluxY_XVZ_cd6
...
contains
  ...
  !> value at XYW
!OCL SERIAL
  subroutine ATMOS_DYN_FVM_flux_valueW_Z_cd6( &
       valW, &
       mflx, val, GSQRT, &
       CDZ )
    implicit none
...
#endif
       valW(k) = F61 * ( val(k+1)+val(k) ) + F62 * ( val(k+2)+val(k-1) ) + F63 * ( val(k+3)+val(k-2) )
    enddo
#ifdef DEBUG
    k = IUNDEF
#endif
       ...
       valW(KS+1) = F41 * ( val(KS+2)+val(KS+1) ) + F42 * ( val(KS+3)+val(KS) )
       valW(KE-2) = F41 * ( val(KE-1)+val(KE-2) ) + F42 * ( val(KE)+val(KE-3) )
  ...
  subroutine ATMOS_DYN_FVM_fluxZ_XYZ_cd6( &
    ...
    do k = KS+2, KE-3
       ...
       vel = mflx(k,i,j)
       flux(k,i,j) = vel &
            * ( F61 * ( val(k+1,i,j)+val(k,i,j) ) + F62 * ( val(k+2,i,j)+val(k-1,i,j) ) + F63 * ( val(k+3,i,j)+val(k-2,i,j) ) )  &
    ...
    do i = IIS, IIE
       ...
       vel = mflx(KS+1,i,j)
       flux(KS+1,i,j) = vel &
            * ( F41 * ( val(KS+2,i,j)+val(KS+1,i,j) ) + F42 * ( val(KS+3,i,j)+val(KS,i,j) ) )  &
            + GSQRT(KS+1,i,j) * num_diff(KS+1,i,j)
       vel = mflx(KE-2,i,j)
       flux(KE-2,i,j) = vel &
            * ( F41 * ( val(KE-1,i,j)+val(KE-2,i,j) ) + F42 * ( val(KE,i,j)+val(KE-3,i,j) ) )  &
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxZ_XYZ_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxX_XYZ_cd6( &
    ...
    do k = KS, KE
       ...
       vel = mflx(k,i,j)
       flux(k,i,j) = vel &
            * ( F61 * ( val(k,i+1,j)+val(k,i,j) ) + F62 * ( val(k,i+2,j)+val(k,i-1,j) ) + F63 * ( val(k,i+3,j)+val(k,i-2,j) ) )  &
            + GSQRT(k,i,j) * num_diff(k,i,j)
    enddo
    enddo
    enddo
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxX_XYZ_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxY_XYZ_cd6( &
    ...
    do k = KS, KE
       ...
       vel = mflx(k,i,j)
       flux(k,i,j) = vel &
            * ( F61 * ( val(k,i,j+1)+val(k,i,j) ) + F62 * ( val(k,i,j+2)+val(k,i,j-1) ) + F63 * ( val(k,i,j+3)+val(k,i,j-2) ) )  &
            + GSQRT(k,i,j) * num_diff(k,i,j)
    enddo
    enddo
    enddo
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxY_XYZ_cd6



  !-----------------------------------------------------------------------------
  !> calculation z-flux at XYW
  subroutine ATMOS_DYN_FVM_fluxZ_XYW_cd6( &
       flux, &
       mom, val, DENS, &
       GSQRT, J33G, &
    ...
    do k = KS+2, KE-2
       ...
       flux(k-1,i,j) = J33G * vel &
            * ( F61 * ( val(k,i,j)+val(k-1,i,j) ) + F62 * ( val(k+1,i,j)+val(k-2,i,j) ) + F63 * ( val(k+2,i,j)+val(k-3,i,j) ) )  &
            + GSQRT(k,i,j) * num_diff(k,i,j)
    enddo
    enddo
    enddo
    ...
    do i = IIS, IIE
       ...
       flux(KS  ,i,j) = J33G * vel &
            * ( F41 * ( val(KS+1,i,j)+val(KS,i,j) ) + F42 * ( val(KS+2,i,j)+val(KS-1,i,j) ) )  &
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxZ_XYW_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxJ13_XYW_cd6( &
    ...
    do i = IIS, IIE
       ...
       vel = ( 0.5_RP * ( mom(KS,i,j)+mom(KS,i-1,j) ) ) &
           / DENS(KS,i,j)
       flux(KS-1,i,j) = J13G(KS,i,j) / MAPF(i,j,+2) * vel &
            * ( F2 * ( val(KS,i,j)+val(KS-1,i,j) ) )
       vel = ( 0.5_RP * ( mom(KE,i,j)+mom(KE,i-1,j) ) ) &
           / DENS(KE,i,j)
       flux(KE-1,i,j) = J13G(KE,i,j) / MAPF(i,j,+2) * vel &
            * ( F2 * ( val(KE,i,j)+val(KE-1,i,j) ) )
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxJ13_XYW_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxJ23_XYW_cd6( &
    ...
    do i = IIS, IIE
       ...
       vel = ( 0.5_RP * ( mom(KS,i,j)+mom(KS,i,j-1) ) ) &
           / DENS(KS,i,j)
       flux(KS-1,i,j) = J23G(KS,i,j) / MAPF(i,j,+1) * vel &
            * ( F2 * ( val(KS,i,j)+val(KS-1,i,j) ) )
       vel = ( 0.5_RP * ( mom(KE,i,j)+mom(KE,i,j-1) ) ) &
           / DENS(KE,i,j)
       flux(KE-1,i,j) = J23G(KE,i,j) / MAPF(i,j,+1) * vel &
            * ( F2 * ( val(KE,i,j)+val(KE-1,i,j) ) )
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxJ23_XYW_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxX_XYW_cd6( &
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxX_XYW_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxY_XYW_cd6( &
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxY_XYW_cd6


  !-----------------------------------------------------------------------------
  !> calculation z-flux at UY
  subroutine ATMOS_DYN_FVM_fluxZ_UYZ_cd6( &
       flux, &
       mom, val, DENS, &
       GSQRT, J33G, &
    ...
    do i = IIS, IIE
       ...
       vel = ( 0.5_RP * ( mom(KS+1,i,j)+mom(KS+1,i+1,j) ) ) &
           / ( F2H(KS+1,1,I_XYZ) * 0.5_RP * ( DENS(KS+2,i,j)+DENS(KS+2,i+1,j) ) &
             + F2H(KS+1,2,I_XYZ) * 0.5_RP * ( DENS(KS+1,i,j)+DENS(KS+1,i+1,j) ) )
       flux(KS+1,i,j) = J33G * vel &
            * ( F41 * ( val(KS+2,i,j)+val(KS+1,i,j) ) + F42 * ( val(KS+3,i,j)+val(KS,i,j) ) )  &
            + GSQRT(KS+1,i,j) * num_diff(KS+1,i,j)
       vel = ( 0.5_RP * ( mom(KE-2,i,j)+mom(KE-2,i+1,j) ) ) &
           / ( F2H(KE-2,1,I_XYZ) * 0.5_RP * ( DENS(KE-1,i,j)+DENS(KE-1,i+1,j) ) &
             + F2H(KE-2,2,I_XYZ) * 0.5_RP * ( DENS(KE-2,i,j)+DENS(KE-2,i+1,j) ) )
       flux(KE-2,i,j) = J33G * vel &
            * ( F41 * ( val(KE-1,i,j)+val(KE-2,i,j) ) + F42 * ( val(KE,i,j)+val(KE-3,i,j) ) )  &
            + GSQRT(KE-2,i,j) * num_diff(KE-2,i,j)
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxZ_UYZ_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxJ13_UYZ_cd6( &
    ...
    do i = IIS, IIE
       ...
       vel = ( F2H(KS+1,1,I_UYZ) * mom(KS+2,i,j) &
             + F2H(KS+1,2,I_UYZ) * mom(KS+1,i,j) ) &
           / ( F2H(KS+1,1,I_XYZ) * 0.5_RP * ( DENS(KS+2,i,j)+DENS(KS+2,i+1,j) ) &
             + F2H(KS+1,2,I_XYZ) * 0.5_RP * ( DENS(KS+1,i,j)+DENS(KS+1,i+1,j) ) )
       flux(KS+1,i,j) = J13G(KS+1,i,j) / MAPF(i,j,+2) * vel &
            * ( F41 * ( val(KS+2,i,j)+val(KS+1,i,j) ) + F42 * ( val(KS+3,i,j)+val(KS,i,j) ) )
       vel = ( F2H(KE-2,1,I_UYZ) * mom(KE-1,i,j) &
             + F2H(KE-2,2,I_UYZ) * mom(KE-2,i,j) ) &
           / ( F2H(KE-2,1,I_XYZ) * 0.5_RP * ( DENS(KE-1,i,j)+DENS(KE-1,i+1,j) ) &
             + F2H(KE-2,2,I_XYZ) * 0.5_RP * ( DENS(KE-2,i,j)+DENS(KE-2,i+1,j) ) )
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxJ13_UYZ_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxJ23_UYZ_cd6( &
    ...
    do i = IIS, IIE
       ...
       vel = ( F2H(KS+1,1,I_XVZ) * 0.25_RP * ( mom(KS+2,i,j)+mom(KS+2,i+1,j)+mom(KS+2,i,j-1)+mom(KS+2,i+1,j-1) ) &
             + F2H(KS+1,2,I_XVZ) * 0.25_RP * ( mom(KS+1,i,j)+mom(KS+1,i+1,j)+mom(KS+1,i,j-1)+mom(KS+1,i+1,j-1) ) ) &
           / ( F2H(KS+1,1,I_XYZ) * 0.5_RP * ( DENS(KS+2,i,j)+DENS(KS+2,i+1,j) ) &
             + F2H(KS+1,2,I_XYZ) * 0.5_RP * ( DENS(KS+1,i,j)+DENS(KS+1,i+1,j) ) )
       flux(KS+1,i,j) = J23G(KS+1,i,j) / MAPF(i,j,+1) * vel &
            * ( F41 * ( val(KS+2,i,j)+val(KS+1,i,j) ) + F42 * ( val(KS+3,i,j)+val(KS,i,j) ) )
       vel = ( F2H(KE-2,1,I_XVZ) * 0.25_RP * ( mom(KE-1,i,j)+mom(KE-1,i+1,j)+mom(KE-1,i,j-1)+mom(KE-1,i+1,j-1) ) &
             + F2H(KE-2,2,I_XVZ) * 0.25_RP * ( mom(KE-2,i,j)+mom(KE-2,i+1,j)+mom(KE-2,i,j-1)+mom(KE-2,i+1,j-1) ) ) &
           / ( F2H(KE-2,1,I_XYZ) * 0.5_RP * ( DENS(KE-1,i,j)+DENS(KE-1,i+1,j) ) &
             + F2H(KE-2,2,I_XYZ) * 0.5_RP * ( DENS(KE-2,i,j)+DENS(KE-2,i+1,j) ) )
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxJ23_UYZ_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxX_UYZ_cd6( &
    ...
    do k = KS, KE
       ...
       flux(k,i-1,j) = GSQRT(k,i,j) / MAPF(i,j,+2) * vel &
            * ( F61 * ( val(k,i,j)+val(k,i-1,j) ) + F62 * ( val(k,i+1,j)+val(k,i-2,j) ) + F63 * ( val(k,i+2,j)+val(k,i-3,j) ) )  &
            + GSQRT(k,i,j) * num_diff(k,i,j)
    enddo
    enddo
    enddo
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxX_UYZ_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxY_UYZ_cd6( &
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxY_UYZ_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxZ_XVZ_cd6( &
       flux, &
       mom, val, DENS, &
       GSQRT, J33G, &
    ...
    do i = IIS, IIE
       ...
       vel = ( 0.5_RP * ( mom(KS+1,i,j)+mom(KS+1,i,j+1) ) ) &
           / ( F2H(KS+1,1,I_XYZ) * 0.5_RP * ( DENS(KS+2,i,j)+DENS(KS+2,i,j+1) ) &
             + F2H(KS+1,2,I_XYZ) * 0.5_RP * ( DENS(KS+1,i,j)+DENS(KS+1,i,j+1) ) )
       flux(KS+1,i,j) = J33G * vel &
            * ( F41 * ( val(KS+2,i,j)+val(KS+1,i,j) ) + F42 * ( val(KS+3,i,j)+val(KS,i,j) ) )  &
            + GSQRT(KS+1,i,j) * num_diff(KS+1,i,j)
       vel = ( 0.5_RP * ( mom(KE-2,i,j)+mom(KE-2,i,j+1) ) ) &
           / ( F2H(KE-2,1,I_XYZ) * 0.5_RP * ( DENS(KE-1,i,j)+DENS(KE-1,i,j+1) ) &
             + F2H(KE-2,2,I_XYZ) * 0.5_RP * ( DENS(KE-2,i,j)+DENS(KE-2,i,j+1) ) )
       flux(KE-2,i,j) = J33G * vel &
            * ( F41 * ( val(KE-1,i,j)+val(KE-2,i,j) ) + F42 * ( val(KE,i,j)+val(KE-3,i,j) ) )  &
            + GSQRT(KE-2,i,j) * num_diff(KE-2,i,j)
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxZ_XVZ_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxJ13_XVZ_cd6( &
    ...
    do i = IIS, IIE
       ...
       vel = ( F2H(KS+1,1,I_UYZ) * 0.25_RP * ( mom(KS+2,i,j)+mom(KS+2,i-1,j)+mom(KS+2,i,j+1)+mom(KS+2,i-1,j+1) ) &
             + F2H(KS+1,2,I_UYZ) * 0.25_RP * ( mom(KS+1,i,j)+mom(KS+1,i-1,j)+mom(KS+1,i,j+1)+mom(KS+1,i-1,j+1) ) ) &
           / ( F2H(KS+1,1,I_XYZ) * 0.5_RP * ( DENS(KS+2,i,j)+DENS(KS+2,i,j+1) ) &
             + F2H(KS+1,2,I_XYZ) * 0.5_RP * ( DENS(KS+1,i,j)+DENS(KS+1,i,j+1) ) )
       flux(KS+1,i,j) = J13G(KS+1,i,j) / MAPF(i,j,+2) * vel &
            * ( F41 * ( val(KS+2,i,j)+val(KS+1,i,j) ) + F42 * ( val(KS+3,i,j)+val(KS,i,j) ) )
       vel = ( F2H(KE-2,1,I_UYZ) * 0.25_RP * ( mom(KE-1,i,j)+mom(KE-1,i-1,j)+mom(KE-1,i,j+1)+mom(KE-1,i-1,j+1) ) &
             + F2H(KE-2,2,I_UYZ) * 0.25_RP * ( mom(KE-2,i,j)+mom(KE-2,i-1,j)+mom(KE-2,i,j+1)+mom(KE-2,i-1,j+1) ) ) &
           / ( F2H(KE-2,1,I_XYZ) * 0.5_RP * ( DENS(KE-1,i,j)+DENS(KE-1,i,j+1) ) &
             + F2H(KE-2,2,I_XYZ) * 0.5_RP * ( DENS(KE-2,i,j)+DENS(KE-2,i,j+1) ) )
       flux(KE-2,i,j) = J13G(KE-2,i,j) / MAPF(i,j,+2) * vel &
            * ( F41 * ( val(KE-1,i,j)+val(KE-2,i,j) ) + F42 * ( val(KE,i,j)+val(KE-3,i,j) ) )
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxJ13_XVZ_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxJ23_XVZ_cd6( &
    ...
    do i = IIS, IIE
       ...
       vel = ( F2H(KS+1,1,I_XVZ) * mom(KS+2,i,j) &
             + F2H(KS+1,2,I_XVZ) * mom(KS+1,i,j) ) &
           / ( F2H(KS+1,1,I_XYZ) * 0.5_RP * ( DENS(KS+2,i,j)+DENS(KS+2,i,j+1) ) &
             + F2H(KS+1,2,I_XYZ) * 0.5_RP * ( DENS(KS+1,i,j)+DENS(KS+1,i,j+1) ) )
       flux(KS+1,i,j) = J23G(KS+1,i,j) / MAPF(i,j,+1) * vel &
            * ( F41 * ( val(KS+2,i,j)+val(KS+1,i,j) ) + F42 * ( val(KS+3,i,j)+val(KS,i,j) ) )
       vel = ( F2H(KE-2,1,I_XVZ) * mom(KE-1,i,j) &
             + F2H(KE-2,2,I_XVZ) * mom(KE-2,i,j) ) &
           / ( F2H(KE-2,1,I_XYZ) * 0.5_RP * ( DENS(KE-1,i,j)+DENS(KE-1,i,j+1) ) &
             + F2H(KE-2,2,I_XYZ) * 0.5_RP * ( DENS(KE-2,i,j)+DENS(KE-2,i,j+1) ) )
       flux(KE-2,i,j) = J23G(KE-2,i,j) / MAPF(i,j,+1) * vel &
            * ( F41 * ( val(KE-1,i,j)+val(KE-2,i,j) ) + F42 * ( val(KE,i,j)+val(KE-3,i,j) ) )
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxJ23_XVZ_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxX_XVZ_cd6( &
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxX_XVZ_cd6
  ...
  subroutine ATMOS_DYN_FVM_fluxY_XVZ_cd6( &
    ...
    do k = KS, KE
       ...
       flux(k,i,j-1) = GSQRT(k,i,j) / MAPF(i,j,+1) * vel &
            * ( F61 * ( val(k,i,j)+val(k,i,j-1) ) + F62 * ( val(k,i,j+1)+val(k,i,j-2) ) + F63 * ( val(k,i,j+2)+val(k,i,j-3) ) )  &
            + GSQRT(k,i,j) * num_diff(k,i,j)
    enddo
    enddo
    enddo
    ...
    return
  end subroutine ATMOS_DYN_FVM_fluxY_XVZ_cd6
...
end module scale_atmos_dyn_fvm_flux_cd6
