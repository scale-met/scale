module scale_atmos_phy_cp_kf_sub
  !-----------------------------------------------------------------------------
  !
  !+++ used modules
  !
  ...
  private :: calcexn
  private :: CONDLOAD_1,CONDLOAD_2
  private :: TPMIX2, DTFRZNEW, PROF5, TPMIX2DD, ENVIRTHT
  private :: ATMOS_THERMODYN_pott_1D
  !-----------------------------------------------------------------------------
  !
  ...
  ! RATE is used subroutine condload1
  ...
  integer ,private, save       :: KF_COND         = 1        ! condload select 1. Ogura and Cho 1973 2. Kessler
  procedure(condload), pointer,private :: p_condload => NULL()
  abstract interface
     subroutine condload &
...
contains
  ...
  subroutine CP_kf_param( & ! set kf tuning parametres
    ...
    if (TRIGGER_in /= 1 .or. TRIGGER_in /=3) then
    ...
    stepkf          = stepkf_in
    if (KF_COND == 1) then
       p_condload => CONDLOAD_1 ! Ogura and Cho
    elseif( KF_COND == 2) then
       p_condload => CONDLOAD_2 ! Kessler type
    else
       write(*,*) 'xxx ERROR at KF namelist'
       write(*,*) 'KF_COND must be 1 or 2'
       call PRC_MPIstop
    end if
    return
...
!          w_cz = 0.02_RP*zlcl/2.e3_RP!
       ...
       if ( TRIGGER == 2) then ! new function none
       else if ( TRIGGER == 3) then ! JMA-NHM trigger function
          ...
          if(U00 < 1._RP) then
            ...
            elseif(rh_lcl > 0.95) then
               dtrh = (1./rh_lcl-1.)*qv_mix/DQSSDT
            else
               dtrh = 0.
            end if
         end if
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  ...
  subroutine CP_kf_updraft (& ! 1d model updraft
    ...
    do kk = k_lclm1,KE-1 ! up_main original(wrf cood K is k_lclm1)
       ...
       call p_condload(qc(kkp1), qi(kkp1), WTW,dztmp, boterm, entterm,qcnew,qinew,qrout(kkp1),&
            & qsout(kkp1),GRAV)
       !! condload_1 KF orginal, condload_2 JMA-NHM type kessler scheme
       !! in original KF cood  'RATE'  is nessesary but this cood late is parameter in subroutine "condload_1"
       ...
       do kk = KS,k_top
          if(qv_g(kk) < 0._RP ) then ! negative moisture
             if(kk == KS) then
                ...
                write(*,*) "@sub scale_atmos_phy_cp_kf_sub",__FILE__,__LINE__
                call PRC_MPIstop
             end if
             kkp1 = kk + 1
             if(kk == k_top) then
...
!@!------------------------------------------------------------------------------
  SUBROUTINE CONDLOAD_1(QLIQ,QICE,WTW,DZ,BOTERM,ENTERM,QNEWLQ,           &
       QNEWIC,QLQOUT,QICOUT,G)
    !original        (QLIQ,QICE,WTW,DZ,BOTERM,ENTERM,RATE,QNEWLQ,           &
    !                          QNEWIC,QLQOUT,QICOUT,G)
    !! RATE is deliate and is local valuable
    ...
    !      real(RP),parameter :: RATE =0.03 ! wrf const
    !
    ...
    QNEWIC=0._RP
    return
  END SUBROUTINE CONDLOAD_1

  ! JMA-NHM like Kessler type auto conversion
  subroutine condload_2 & 
       (QLIQ,QICE,WTW,DZ,BOTERM,ENTERM,QNEWLQ,           &
        QNEWIC,QLQOUT,QICOUT,G)
     implicit none
    ! kesseler type
...
!      aintrp=(a1-value)/ainc
  ...
  !
  subroutine ATMOS_THERMODYN_pott_1D (& ! calc potential temperature from scale_atmos_sub_thermodyn.F90
       pott, &
       temp, &
       pres, &
       q     )
    ...
    integer kk
    do kk= KS,KE
       call ATMOS_THERMODYN_pott (pott(kk),temp(kk),pres(kk),q(kk,:))
    end do
    return
  end subroutine ATMOS_THERMODYN_pott_1D
  
end module scale_atmos_phy_cp_kf_sub
