...
!!          Bulk Method
!!          Produced based on scale_atmos_phy_sf_louis.F90
!!
!! @author Team SCALE
!!
!! @par History
!! @li      2014-04-15 (A.Noda)      [new]
!<
!-------------------------------------------------------------------------------
module scale_atmos_phy_sf_grayzone
  !-----------------------------------------------------------------------------
  !
  !++ used modules
  !
  ...
  !++ Public procedure
  !
  public :: ATMOS_PHY_SF_grayzone_setup
  public :: ATMOS_PHY_SF_grayzone
  ...
  integer :: mstep=30
! real(RP), save :: time_in(mstep)
! real(RP), save :: lhf_in(mstep)
! real(RP), save :: shf_in(mstep)
...
! real(DP),save,allocatable :: sst_loc(:,:)
...
contains
  ...
  !
  !-----------------------------------------------------------------------------
  subroutine ATMOS_PHY_SF_grayzone_setup( ATMOS_TYPE_PHY_SF )
    use scale_process, only: &
       PRC_MPIstop
    implicit none

    character(100) :: inbasedir = './'
    character(100) :: fdata_name = 'srf_flux_force.txt'
    integer :: fid_data
    ...
    NAMELIST / PARAM_ATMOS_PHY_SF_GRAYZONE / &
!      fdata_name_sst,&
!      mstep_sst,         &
       GIVEN_HEAT_FLUX
    ...
    if( IO_L ) write(IO_FID_LOG,*) '*** Grayzone parameterization'

    if ( ATMOS_TYPE_PHY_SF /= 'GRAYZONE' ) then
       if ( IO_L ) write(IO_FID_LOG,*) 'xxx ATMOS_TYPE_PHY_SF is not GRAYZONE. Check!'
       call PRC_MPIstop
    endif

    !--- read namelist
    rewind(IO_FID_CONF)
    read(IO_FID_CONF,nml=PARAM_ATMOS_PHY_SF_GRAYZONE,iostat=ierr)

    if( ierr < 0 ) then !--- missing
       if( IO_L ) write(IO_FID_LOG,*) '*** Not found namelist. Default used.'
    elseif( ierr > 0 ) then !--- fatal error
       write(*,*) 'xxx Not appropriate names in namelist PARAM_ATMOS_PHY_SF_GRAYZONE. Check!'
       call PRC_MPIstop
    endif
    if( IO_L ) write(IO_FID_LOG,nml=PARAM_ATMOS_PHY_SF_GRAYZONE)
...
!     close(fid_data)
    ...
    if( GIVEN_HEAT_FLUX )then
      ...
      read(fid_data,*)
      do t=1, mstep
        read(fid_data,*) time_in(t), lhf_in(t), shf_in(t)
      enddo
      close(fid_data)
    endif

    return
  end subroutine ATMOS_PHY_SF_grayzone_setup

  !-----------------------------------------------------------------------------
  ! calclate surface flux
  !-----------------------------------------------------------------------------
  subroutine ATMOS_PHY_SF_grayzone( &
...
!         sst_loc(:,:)=( (time_sst_in(t+1)-time_nowsec)*sst_in(t)+(time_nowsec-time_sst_in(t))*sst_in(t+1) )&
...
!           ( SST_loc(i,j)+SST(1,i+1,j) ) * 0.5_RP,     & ! (in)
            Uabs**2,                              & ! (in)
            CZ(KS), Z0, Zt,                       & ! (in)
            KARMAN, FB, FBS, FDM, FDH,            & ! (in)
            ThS, GRAV                             ) ! (in)
...
end module scale_atmos_phy_sf_grayzone
