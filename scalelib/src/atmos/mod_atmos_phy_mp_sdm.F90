...
!  use mod_index
  ...
  real(RP), allocatable :: sdz_s2c(:)      ! z-cordinate of s.d.
  real(RP), allocatable :: sdr_s2c(:)      ! y-cordinate of s.d.
  real(RP), allocatable :: sdu_s2c(:)      ! x-components velocity of s.d.
  real(RP), allocatable :: sdv_s2c(:)      ! y-components velocity of s.d.
  real(RP), allocatable :: sdvz_s2c(:)     ! z-components velocity of s.d. and terminal veloicty of s.d.
  real(RP), allocatable :: sdasl_s2c(:,:)  ! aeosol mass of s.d.
  real(RP), allocatable :: sdrkl_s2c(:,:)  ! index-k(real) at 'sdm_zlower'
  real(RP), allocatable :: sdrku_s2c(:,:)  ! index-k(real) at 'sdm_upper'
  integer(DP), allocatable :: sdn_tmp(:)   ! multiplicity of super-droplets
  real(RP), allocatable :: sdrk_tmp(:)     ! index-k(real) of super-droplets
  real(RP), allocatable :: sdx_tmp(:)      ! x-coordinate of super-droplets
  real(RP), allocatable :: sdy_tmp(:)      ! y-coordinate of super-droplets
  real(RP), allocatable :: sdz_tmp(:)      ! z-coordinate of super-droplets
  ...
  real(RP), allocatable :: sdrk_fm(:)      ! index-k(real) of super-droplets
  real(RP), allocatable :: sdx_fm(:)       ! x-coordinate of super-droplets
  ...
  real(RP), allocatable :: rhor_sdm(:,:,:) ! density of rain water
  real(RP), allocatable :: rhoa_sdm(:,:,:) ! density of aerosol
  real(RP), allocatable :: rand_s2c(:)
  integer, allocatable :: sortid_s2c(:)
  integer, allocatable :: sortfreq_s2c(:)
  integer, allocatable :: sortkey_s2c(:)
  integer, allocatable :: sorttag_s2c(:)
  real(DP), allocatable :: rbuf(:,:,:)
  real(DP), allocatable :: sbuf(:,:,:)
  integer, allocatable :: sdm_itmp1(:)
  integer, allocatable :: sdm_itmp2(:)
  integer, allocatable :: sdm_itmp3(:)
  integer, allocatable :: sd_itmp1(:,:)
  integer, allocatable :: sd_itmp2(:,:)
  integer, allocatable :: sd_itmp3(:,:)
  real(RP), allocatable :: sd_dtmp1(:)
  !------------------------------------------------------------------------------
  !
  !++ For Random number
  !
  !------------------------------------------------------------------------------
  integer :: randseed_s2c
  !------------------------------------------------------------------------------
  !
  !++ Basic variable for SDM
  !
...
!  integer, private :: ni=IE-IS+1, nj=JE-JS+1, nz=KE-KS+1     ! Number of grid for each dirrection with halo
  ...
  integer, private :: srcw_sub, srce_sub, srcs_sub, srcn_sub
  !------------------------------------------------------------------------------
  !
  !++ Basic parameter for SDM
  !
  !------------------------------------------------------------------------------
  real(RP), private, parameter :: LatHet = 2.453E+6_RP   ! Latent heat of water at 293K [J/kg]
  real(RP), private, parameter :: Heat_C = 2.550E-2_RP   ! thermal conductivity at 293K, 100kPa [J/m s K]
  real(RP), private, parameter :: DNS_RL = 998.203_RP     ! Density of liquid water at 293K [kg m-3]
  real(RP), private, parameter :: Diff_C = 2.52E-5_RP    ! Diffusion Constant
  real(RP), private, parameter :: LatGas = LatHet / GasV_C
  real(RP), private, parameter :: L_RL_K = LatHet * DNS_RL / Heat_C
  real(RP), private, parameter :: RLRv_D = DNS_RL * GasV_C / Diff_C
!  real(RP), private, parameter :: exp_K = GasD_C / cp_C    ! exponent k used in adiabatic process
  real(RP), private, parameter :: m2micro = 1.0E+6_RP    ! Convert unit [m] -> [micron]
  real(RP), private, parameter :: micro2m = 1.0E-6_RP    ! Convert unit [micron] -> [m]
  real(RP), private, parameter :: TWO_PI = 2.0_RP * 3.14159265358979_RP
  real(RP), private, parameter :: O_THRD = 1.0_RP / 3.0_RP
  ...
  real(RP), private  :: n3_nacl
  real(RP), private  :: rb1_amsul                                 !! [m]
  real(RP), private  :: rb2_amsul
  real(RP), private  :: rb3_nacl
  real(RP), private  :: sgm1_amsul                                !! [-]
  real(RP), private  :: sgm2_amsul
  real(RP), private  :: sgm3_nacl
  real(RP), private  :: rmax_amsul                                !! [m]
  real(RP), private  :: rmax_nacl
  real(RP), private  :: rmin_amsul
  real(RP), private  :: rmin_nacl
  ! parameter for 3mode log-normal distribution under LICO-observation : Derksen(2009)
  real(RP), private, parameter :: n1_amsul_derksn   = 118.E+6_RP   !! [1/m3]
  real(RP), private, parameter :: n2_amsul_derksn   = 11.E+6_RP
  real(RP), private, parameter :: n3_nacl_derksn    = 0.1E+6_RP
  real(RP), private, parameter :: rb1_amsul_derksn  = 1.9E-08_RP   !! [m]
  real(RP), private, parameter :: rb2_amsul_derksn  = 5.6E-08_RP
  real(RP), private, parameter :: rb3_nacl_derksn   = 8.0E-07_RP
  real(RP), private, parameter :: sgm1_amsul_derksn = 3.3_RP       !! [-]
  real(RP), private, parameter :: sgm2_amsul_derksn = 1.6_RP
  real(RP), private, parameter :: sgm3_nacl_derksn  = 2.2_RP
  real(RP), private, parameter :: rmax_amsul_derksn = 5.0E-06_RP   !! [m]
  real(RP), private, parameter :: rmax_nacl_derksn  = 5.0E-06_RP
  real(RP), private, parameter :: rmin_amsul_derksn = 2.0E-09_RP
  real(RP), private, parameter :: rmin_nacl_derksn  = 1.0E-07_RP
  ! parameter for 3mode log-nomiral distribution under LICO-observation : vanZanten(2010)
  real(RP), private, parameter :: n1_amsul_zanten   = 90.E+6_RP    !! [1/m3]
  real(RP), private, parameter :: n2_amsul_zanten   = 15.E+6_RP
  real(RP), private, parameter :: rb1_amsul_zanten  = 3.0E-08_RP   !! [m]
  real(RP), private, parameter :: rb2_amsul_zanten  = 1.4E-07_RP
  real(RP), private, parameter :: sgm1_amsul_zanten = 1.28_RP      !! [-]
  real(RP), private, parameter :: sgm2_amsul_zanten = 1.75_RP
  real(RP), private, parameter :: rmax_amsul_zanten = 5.0E-06_RP   !! [m]
  real(RP), private, parameter :: rmin_amsul_zanten = 1.0E-08_RP
  ! parameter for 3mode log-nomiral distribution under LICO-observation : modified vanZanten(2010)
  real(RP), private, parameter :: rb_amsul_zanten_m   = 3.0E-08_RP   !! [m]
  real(RP), private, parameter :: sgm_amsul_zanten_m  = 1.28_RP      !! [-]
  real(RP), private, parameter :: rmax_amsul_zanten_m = 1.0E-07_RP   !! [m]
  real(RP), private, parameter :: rmin_amsul_zanten_m = 1.0E-08_RP
  real(RP), private, parameter :: Es_T_A = 2.53E+11_RP  ! A[kg/m s2] : Es = A * exp(-B/T)
  real(RP), private, parameter :: Es_T_B = 5.42E+3_RP   ! B[K] : Es = A * exp(-B/T)
  real(RP), private, parameter :: CurveF = 3.3E-7_RP    !  Curvature term Const. a=CoruveF/T [m K]
  real(RP), private, parameter :: ASL_FF = 4.3E-6_RP    !  Cloud Condensation term Const. b=ASL_FF*(Ion*M)/ms [m3]
  !------------------------------------------------------------------------------
  !
  !++ Terminal velocity
  !
  !------------------------------------------------------------------------------
  real(RP), private :: vz_b(7), vz_c(6)
  ...
  !
  !------------------------------------------------------------------------------
  real(RP), private :: ratcol(21), r0col(15), ecoll(15,21)
  ...
  !
  !------------------------------------------------------------------------------
  real(RP), private :: sdm_dtcmph(3)              ! Time interval [sec] of condensation, coalescence, and droplet motion
  ...
  real(RP), private :: sdm_rqc2qr   = 1.E-5_RP    ! Threshold of radius between qc and qr [m]
  integer,  private :: sdm_colkrnl  = 2           ! Kernel type for coalescence process (0:Golovin, 1:Long, 2:Halls,3:No col_effi hydrodynamic)
  ...
  data sdm_aslrho / 0.0_RP, 0.0_RP, 0.0_RP, 0.0_RP, 0.0_RP, &
                    ...
                    0.0_RP, 0.0_RP, 0.0_RP, 0.0_RP, 0.0_RP, &
                    0.0_RP, 0.0_RP, 0.0_RP, 0.0_RP, 0.0_RP  /
  real(RP), allocatable  :: zph_crs(:,:,:), dxiv_sdm(:), dyiv_sdm(:), dziv_sdm(:)
  real(RP),private,allocatable :: dx_sdm(:), dy_sdm(:), dz_sdm(:)   ! Dx, Dy, Dz for SDM (normally they are equal to those of SCALE)
!  integer, private, parameter :: nqw = QQA
  integer, private :: sdfmnum_s2c
  real(RP),private :: sdininum_s2c
  integer, private :: sdnumasl_s2c, sdnum_s2c
  integer, private :: ni_s2c, nj_s2c, nk_s2c
  integer, private :: tag
  integer, private, parameter :: nomp = 1
  integer, private :: wbc=1, ebc=1, sbc=1, nbc=1  ! only periodic boundary is applied
  integer, private :: nsub   ! Number of sub domain in group domain
...
!  real(RP), private :: crs_dtmp5(KA,IA,JA), crs_dtmp6(KA,IA,JA)
  ...
  real(RP), private :: sdm_dtadv  ! time step of {motion of super-droplets} process
!  real(RP), private, allocatable :: qwtr_crs(:,:,:,:)
  real(RP), allocatable :: j31(:,:,:) ! z-x components of Jacobian
  real(RP), allocatable :: j32(:,:,:) ! z-y components of Jacobian
  real(RP), allocatable :: jcb(:,:,:) ! Jacobian at scalar points
  real(RP), allocatable :: jcb8w(:,:,:) ! Jacobian at w points
  real(RP), allocatable :: mf(:,:)  ! Map scale factors
  integer,  allocatable :: KMIN1(:), IMIN1(:), JMIN1(:)
  integer,  allocatable :: KPLS1(:), IPLS1(:), JPLS1(:)
  integer,  private :: nclstp(0:3)

  logical, private, save :: docondensation = .true.
  logical, private, save :: doautoconversion = .true.
  logical, private, save :: doprecipitation  = .true.
  logical, private, save :: domovement = .true.
...
!      MOMZ(k,i,j) = wcf_crs(k,i,j) * 0.5_RP * ( DENS(k,i,j)+DENS(KPLS1(k),i,j) ) / jcb8w(k,i,j)
  ...
  subroutine sdm_numset(         &
      ...
      real(RP),intent(in) :: sdm_zlower    ! Lowest limitaio of initial super droplets distribution
      real(RP),intent(in) :: sdm_zupper    ! Highest limitaion of initial super droplets distribution
      real(RP),intent(out) :: minzph
      real(RP),intent(out) :: sdininum_s2c
      integer,intent(out) :: sdfmnum_s2c
      integer, intent(out) :: sdnum_s2c
      integer, intent(out) :: ni_s2c, nj_s2c, nk_s2c
      real(RP),intent(inout) :: zph_crs(KA,IA,JA)
      ! Work variables
      real(RP) :: bufsiz
...
!      real(RP) :: dyiv_sdm   ! Inverse of dy_sdm
...
!     &                           sd_rkl,sd_rku)
            ...
            call sdm_boundry(wbc,ebc,sbc,nbc,                           &
                             sd_num,sd_numasl,sd_n,sd_x,sd_y,sd_rk,     &
...
!      call cpondpe   !--- do nothing
  ...
  subroutine sdm_putbufsx(wbc,ebc,sd_num,sd_numasl,         &
     ...
     use mod_process, only: &
      ...
      ! Work variables
      integer :: nesend(1:nomp)   ! list number
      integer :: nwsend(1:nomp)   ! list number
      integer :: maxesend         ! maximum list number
      integer :: maxwsend         ! maximum list number
  ...
  subroutine sdm_getbufsx(wbc,ebc,sd_num,sd_numasl,         &
      ...
      integer :: np               ! index for OpenMP
      ! Work variables
      integer :: nerecv(1:nomp)   ! list number
      integer :: nwrecv(1:nomp)   ! list number
      integer :: nesend(1:nomp)   ! list number
      integer :: nwsend(1:nomp)   ! list number

      integer :: minerecv         ! minimum list number
      integer :: minwrecv         ! minimum list number
      integer :: maxesend         ! maximum list number
      integer :: maxwsend         ! maximum list number
  ...
  subroutine sdm_putbufsy(sbc,nbc,sd_num,sd_numasl,         &
      ...
      integer :: np               ! index for OpenMP
      ! Work variables
      integer :: nssend(1:nomp)   ! list number
      integer :: nnsend(1:nomp)   ! list number
      integer :: maxssend         ! maximum list number
      integer :: maxnsend         ! maximum list number
...
!
  ...
  subroutine sdm_getbufsy(sbc,nbc,sd_num,sd_numasl,         &
      ...
      ! Work variables
      integer :: nsrecv(1:nomp)   ! list number
      integer :: nnrecv(1:nomp)   ! list number
      integer :: nssend(1:nomp)   ! list number
      integer :: nnsend(1:nomp)   ! list number

      integer :: minsrecv         ! minimum list number
      integer :: minnrecv         ! minimum list number
      integer :: maxssend         ! maximum list number
      integer :: maxnsend         ! maximum list number
