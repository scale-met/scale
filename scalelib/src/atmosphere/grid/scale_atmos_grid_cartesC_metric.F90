...
contains
  ...
  subroutine ATMOS_GRID_CARTESC_METRIC_mapfactor
    ...
    call MAPPROJECTION_mapfactor( ATMOS_GRID_CARTESC_REAL_LATU (1:IA,1:JA), ATMOS_GRID_CARTESC_METRIC_MAPF(:,:,1,I_UY), ATMOS_GRID_CARTESC_METRIC_MAPF (:,:,2,I_UY))
    call MAPPROJECTION_mapfactor( ATMOS_GRID_CARTESC_REAL_LATV (1:IA,1:JA), ATMOS_GRID_CARTESC_METRIC_MAPF(:,:,1,I_XV), ATMOS_GRID_CARTESC_METRIC_MAPF (:,:,2,I_XV))
  ...
  subroutine ATMOS_GRID_CARTESC_METRIC_terrainfollowing
    ...
    use scale_atmos_grid_cartesC_real, only: &
       ...
       ATMOS_GRID_CARTESC_REAL_CZU,  &
       ATMOS_GRID_CARTESC_REAL_CZV,  &
       ATMOS_GRID_CARTESC_REAL_CZUV, &
       ATMOS_GRID_CARTESC_REAL_FZ,   &
       ATMOS_GRID_CARTESC_REAL_FZU,  &
       ATMOS_GRID_CARTESC_REAL_FZV,  &
    ...
    do i = IS, IE
       ...
       ! at (u,y,w)
       do k = 1, KA-1
          ATMOS_GRID_CARTESC_METRIC_GSQRT(k,i,j,I_UYW) = ( ATMOS_GRID_CARTESC_REAL_CZU(k+1,i,j) - ATMOS_GRID_CARTESC_REAL_CZU(k,i,j) ) * ATMOS_GRID_CARTESC_RFDZ(k)
       enddo
       ATMOS_GRID_CARTESC_METRIC_GSQRT(KA,i,j,I_UYW) = ATMOS_GRID_CARTESC_METRIC_GSQRT(KA-1,i,j,I_UYW)

       ! at (x,v,w)
       do k = 1, KA-1
          ATMOS_GRID_CARTESC_METRIC_GSQRT(k,i,j,I_XVW) = ( ATMOS_GRID_CARTESC_REAL_CZV(k+1,i,j) - ATMOS_GRID_CARTESC_REAL_CZV(k,i,j) ) * ATMOS_GRID_CARTESC_RFDZ(k)
       enddo
       ATMOS_GRID_CARTESC_METRIC_GSQRT(KA,i,j,I_XVW) = ATMOS_GRID_CARTESC_METRIC_GSQRT(KA-1,i,j,I_XVW)

       ! at (u,y,z)
       do k = 1, KA
          ATMOS_GRID_CARTESC_METRIC_GSQRT(k,i,j,I_UYZ) = ( ATMOS_GRID_CARTESC_REAL_FZU(k,i,j) - ATMOS_GRID_CARTESC_REAL_FZU(k-1,i,j) ) * ATMOS_GRID_CARTESC_RCDZ(k)
       enddo

       ! at (x,v,z)
       do k = 1, KA
          ATMOS_GRID_CARTESC_METRIC_GSQRT(k,i,j,I_XVZ) = ( ATMOS_GRID_CARTESC_REAL_FZV(k,i,j) - ATMOS_GRID_CARTESC_REAL_FZV(k-1,i,j) ) * ATMOS_GRID_CARTESC_RCDZ(k)
       enddo
    ...
    do i = IS, IE
    do k = 1,  KA
       ATMOS_GRID_CARTESC_METRIC_J13G(k,i,j,I_XYZ) = -( ATMOS_GRID_CARTESC_REAL_CZU (k,i  ,j) - ATMOS_GRID_CARTESC_REAL_CZU (k,i-1,j) ) * ATMOS_GRID_CARTESC_RCDX(i)
       ATMOS_GRID_CARTESC_METRIC_J13G(k,i,j,I_XYW) = -( ATMOS_GRID_CARTESC_REAL_FZU (k,i  ,j) - ATMOS_GRID_CARTESC_REAL_FZU (k,i-1,j) ) * ATMOS_GRID_CARTESC_RCDX(i)
       ...
       ATMOS_GRID_CARTESC_METRIC_J13G(k,i,j,I_UVZ) = -( ATMOS_GRID_CARTESC_REAL_CZV (k,i+1,j) - ATMOS_GRID_CARTESC_REAL_CZV (k,i  ,j) ) * ATMOS_GRID_CARTESC_RFDX(i)
    enddo
    enddo
    enddo
    ...
    do i = IS, IE
    do k = 1,  KA
       ATMOS_GRID_CARTESC_METRIC_J23G(k,i,j,I_XYZ) = -( ATMOS_GRID_CARTESC_REAL_CZV (k,i,j  ) - ATMOS_GRID_CARTESC_REAL_CZV (k,i,j-1) ) * ATMOS_GRID_CARTESC_RCDY(j)
       ATMOS_GRID_CARTESC_METRIC_J23G(k,i,j,I_XYW) = -( ATMOS_GRID_CARTESC_REAL_FZV (k,i,j  ) - ATMOS_GRID_CARTESC_REAL_FZV (k,i,j-1) ) * ATMOS_GRID_CARTESC_RCDY(j)
       ...
       ATMOS_GRID_CARTESC_METRIC_J23G(k,i,j,I_UVZ) = -( ATMOS_GRID_CARTESC_REAL_CZU (k,i,j+1) - ATMOS_GRID_CARTESC_REAL_CZU (k,i,j  ) ) * ATMOS_GRID_CARTESC_RFDY(j)
    enddo
    enddo
    enddo
