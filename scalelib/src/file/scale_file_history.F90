...
contains
  ...
  subroutine FILE_HISTORY_Setup( &
    ...
    if( IO_L ) write(IO_FID_LOG,*) '*** Output default data type                     : ', FILE_HISTORY_DEFAULT_DATATYPE
  ...
  subroutine FILE_HISTORY_reg( &
    ...
    if ( FILE_HISTORY_dims(dimid)%nzcoords > 0 ) then
       ...
       do n = 1, FILE_HISTORY_dims(dimid)%nzcoords
          ...
          call FILE_HISTORY_Add_Variable( itemid,                              & ! (out)
                                          name, desc, unit,                    & ! (in)
                                          dimid,                               & ! (in)
  ...
  subroutine FILE_HISTORY_in_0D( &
    ...
    call FILE_HISTORY_reg( itemid,           & ! [OUT]
                           name, desc, unit, & ! [IN]
                           dim_type,         & ! [IN]
                           fill_halo,        & ! [IN]
                           ndim              ) ! [IN]
  ...
  subroutine FILE_HISTORY_Set_Dim( &
    ...
    FILE_HISTORY_dims(id)%count(1:ndims) = count(:)
    FILE_HISTORY_dims(id)%size           = size
  ...
  subroutine FILE_HISTORY_Set_Axis( &
    ...
    if ( present(down) ) then
       FILE_HISTORY_axes(FILE_HISTORY_naxes)%down = down
    else
    ...
    if ( present(start) ) then  ! global subarray starting indices
       ...
       FILE_HISTORY_axes(FILE_HISTORY_naxes)%start(1) = 1
    end if
       ...
       if ( .NOT. fileexisted ) then ! new file
          ...
          if ( FILE_HISTORY_io_buffer_size == 0 ) then
             ...
             do m = 1, FILE_HISTORY_nitems
                ...
                array_size = array_size + FILE_HISTORY_dims(dimid)%size
             end do
             FILE_HISTORY_io_buffer_size = array_size * FILE_HISTORY_nitems * DP
          end if
  ...
  subroutine FILE_HISTORY_Write_Axes
    ...
    do id = 1, FILE_HISTORY_nitems
       ...
       do m = 1, FILE_HISTORY_naxes
          if ( FILE_HISTORY_axes(m)%start(1) > 0 ) then
             start(1) = FILE_HISTORY_axes(m)%start(1)
  ...
  subroutine FILE_HISTORY_Write_OneVar( &
    ...
    if ( step_now > FILE_HISTORY_vars(id)%waitstep ) then
       ...
       dimid = FILE_HISTORY_vars(id)%dimid
       if ( FILE_HISTORY_dims(dimid)%count(1) > 0 ) then
          ...
          call FILE_Write( FILE_HISTORY_vars(id)%vid,             & ! [IN]
                           ...
                           count=FILE_HISTORY_dims(dimid)%count,  & ! global subarray lengths
                           start=FILE_HISTORY_dims(dimid)%start   ) ! global subarray start indices
       end if
    else
