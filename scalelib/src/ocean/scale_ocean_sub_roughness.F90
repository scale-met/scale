...
contains
  ...
  subroutine OCEAN_roughness_miller92( &
    ...
    do j = JS, JE
    do i = IS, IE
       Ustar = max( sqrt(CM0) * ATM_Uabs(i,j), OCEAN_roughness_Ustar_min )
  ...
  subroutine OCEAN_roughness_moon07( &
    ...
    do ite = 1, itelim
       do j = JS, JE
       do i = IS, IE
          Ustar(i,j) = KARMAN * ATM_Uabs(i,j) / log( Z1(i,j)/Z0M(i,j) )
       enddo
       enddo
       ...
       do i = IS, IE
          ...
          if ( U10M <= 12.5_RP ) then
             ...
             Z0M(i,j) = 1.0E-3_RP * ( 0.085_RP * (  -0.56_RP*Ustar(i,j)*Ustar(i,j) &
                                                 + 20.255_RP*Ustar(i,j)            &
                                                 +  2.458_RP                       ) - 0.58_RP )
          endif
       enddo
