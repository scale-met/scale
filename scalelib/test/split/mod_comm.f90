module mod_comm
  ...
  subroutine comm_makeyp ( nprocs_p, nprocs_c, handle )
    ...
    if ( handle == parent ) then !--- parent
       ...
       call MPI_BCAST( yplist_p, ileng, MPI_INTEGER, master, LOC_COMM_WORLD, ierr )
       call MPI_BCAST( ypall_p,  1,     MPI_INTEGER, master, LOC_COMM_WORLD, ierr )
    ...
    elseif ( handle == child ) then !--- child
       ...
       call MPI_BCAST( yplist_c, ileng, MPI_INTEGER, master, LOC_COMM_WORLD, ierr )
       call MPI_BCAST( ypall_c,  1,     MPI_INTEGER, master, LOC_COMM_WORLD, ierr )
  ...
  subroutine comm_abort ()
    ...
    if ( masternode ) write ( MFID, * ) "COMM/ABORT: ERROR"

    call MPI_ABORT(GLB_COMM_WORLD, 1, ierr)
