...
!          mflx_hi(k,i,j,YDIR) = ( -MOMY(k,i,j+1) + 8.0_RP*MOMY(k,i,j) -MOMY(k,i,j-1) ) / 6.0_RP &
    ...
    IIE = IIS+IBLOCK-1
...
#endif
          qflx_hi(k-1,i,j,ZDIR) = 0.5_RP * ( MOMZ(k,i,j)+MOMZ(k-1,i,j) ) / DENS(k,i,j) &
       ...
       do i = IIS, IIE
          ...
          qflx_hi(KS,i,j,ZDIR) = sign( 1.0_RP, MOMZ(KS,i,j) ) &
                               * min( sign( 1.0_RP, MOMZ(KS,i,j) ) &
                                    * ( vel * ( FACT_N * ( MOMZ(KS+1,i,j)+MOMZ(KS,i,j) ) &
                                              + FACT_F * ( MOMZ(KS+2,i,j)              ) ) &
                               + num_diff(KS+1,i,j,I_MOMZ,ZDIR) ), &
                                 abs(MOMZ(KS,i,j)) * FDZ(KS) / dtrk )
...
!                              + num_diff(KS+1,i,j,I_MOMZ,ZDIR)
          ...
          qflx_hi(KE-2,i,j,ZDIR) = vel * ( FACT_N * ( MOMZ(KE-1,i,j)+MOMZ(KE-2,i,j) ) &
...
#endif
          qflx_hi(k,i,j,ZDIR) = 0.5_RP * ( VELZ(k,i+1,j)+VELZ(k,i,j) ) &
...
#endif
          qflx_hi(k,i,j,ZDIR) = 0.5_RP * ( VELZ(k,i,j+1)+VELZ(k,i,j) ) &
...
#endif
          qflx_hi(k,i,j,ZDIR) = mflx_hi(k,i,j,ZDIR) * ( FACT_N * ( POTT(k+1,i,j)+POTT(k  ,i,j) ) &
...
#endif
          qflx_hi(KS-1,i,j,ZDIR) = 0.0_RP
          qflx_hi(KS  ,i,j,ZDIR) = 0.5_RP * mflx_hi(KS  ,i,j,ZDIR) * ( POTT(KS+1,i,j)+POTT(KS,i,j) ) & ! just above the bottom boundary
                                 + num_diff(KS  ,i,j,I_RHOT,ZDIR)
          qflx_hi(KE-1,i,j,ZDIR) = 0.5_RP * mflx_hi(KE-1,i,j,ZDIR) * ( POTT(KE,i,j)+POTT(KE-1,i,j) ) & ! just below the top boundary
...
#endif
          qflx_hi(k,i,j,XDIR) = mflx_hi(k,i,j,XDIR) * ( FACT_N * ( POTT(k,i+1,j)+POTT(k,i  ,j) ) &
...
#endif
          qflx_hi(k,i,j,YDIR) = mflx_hi(k,i,j,YDIR) * ( FACT_N * ( POTT(k,i,j+1)+POTT(k,i,j  ) ) &
