  ...
  subroutine ATMOS_PHY_MP
    ...
    do i = IS, IE
       ...
       do k = KS, KE
          ...
          Rmoist (k) = Rdry  +  Rvap * QTRC(k,i,j,I_QV)
          CPmoist(k) = CPdry + CPvap * QTRC(k,i,j,I_QV)
          CVmoist(k) = CVdry + CVvap * QTRC(k,i,j,I_QV)

          pres(k) = P00 * ( DENS(k,i,j) * pott(k) * Rdry / P00 )**CPovCV
          temp(k) = pres(k) / ( DENS(k,i,j) * Rdry )
...
!          endif
    ...
    call COMM_vars8( QTRC(:,:,:,I_QC), 3 )
    call COMM_vars8( QTRC(:,:,:,I_QR), 4 )
    call COMM_wait ( RHOT(:,:,:),      1 )
    call COMM_wait ( QTRC(:,:,:,I_QV), 2 )
    call COMM_wait ( QTRC(:,:,:,I_QC), 3 )
    call COMM_wait ( QTRC(:,:,:,I_QR), 4 )
