...
module mod_const
  ...
  real(8), public, save      :: CONST_CVair                !< specific heat (dry,constant volume)
...
contains
  ...
  subroutine CONST_setup
    ...
    CONST_CVair   = CONST_CPair - CONST_Rair

    CONST_RovCP   = CONST_Rair  / CONST_CPair
    CONST_CPovR   = CONST_CPair / CONST_Rair
    CONST_CPovCV  = CONST_CPair / CONST_CVair
    CONST_CVovCP  = CONST_CVair / CONST_CPair

    CONST_EPSvap  = CONST_Rair / CONST_Rvap
    ...
    if( IO_L ) write(IO_FID_LOG,*) '*** R(dry air)  :', CONST_Rair
    if( IO_L ) write(IO_FID_LOG,*) '*** CP(air)     :', CONST_CPair
    if( IO_L ) write(IO_FID_LOG,*) '*** CV(air)     :', CONST_CVair
