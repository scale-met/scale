...
module mod_atmos_dyn
  ...
  real(8), private, save :: rjpls   (KA,IA,JA)   ! correction factor for incoming in (x,y,z)-direction
...
!OCL XFILL
  ...
  subroutine ATMOS_DYN
    ...
    real(8) :: pjmax, pjmin, tmp
    real(8) :: pjpls(KA,IA,JA)
    real(8) :: pjmns(KA,IA,JA)

    ! idivergence damping 
    real(8) :: div (KA,IA,JA)     ! divergence
!    real(8), parameter :: divdmp_coef = 1.D-3
    real(8) :: divdmp_coef
...
call START_COLLECTION("DYNAMICS")
    ...
    divdmp_coef = ATMOS_DYN_divdmp_coef
...
!          div (k,i,j) = ( mflx_hi(k,i,j,ZDIR)-mflx_hi(k-1,i,  j,  ZDIR) ) * RCDZ(k) &
!                      + ( mflx_hi(k,i,j,XDIR)-mflx_hi(k  ,i-1,j,  XDIR) ) * RCDX(i) &
       ...
       do k = KS, KE-1
          MOMZ_RK1(k,i,j) = MOMZ(k,i,j) &
                          + dtrk * ( - ( ( qflx_hi(k+1,i,j,ZDIR)-qflx_hi(k,i  ,j  ,ZDIR) ) * RFDZ(k) &
                                     ...
                                     + divdmp_coef*FDZ(k)*dtrk * ( div(k+1,i,j)-div(k,i,j) )           & ! divergence damping
       ...
       do k = KS, KE
          MOMX_RK1(k,i,j) = MOMX(k,i,j) &
                          + dtrk * ( - ( ( qflx_hi(k,i  ,j,ZDIR)-qflx_hi(k-1,i,j,  ZDIR) ) * RCDZ(k) &
                                     ...
                                     + divdmp_coef*FDX(i)*dtrk * ( div(k,i+1,j)-div(k,i,j) )           & ! divergence damping
       ...
       do k = KS, KE
          MOMY_RK1(k,i,j) = MOMY(k,i,j) &
                          + dtrk * ( - ( ( qflx_hi(k,i,j  ,ZDIR)-qflx_hi(k-1,i  ,j,ZDIR) ) * RCDZ(k) &
                                     ...
                                     + divdmp_coef*FDY(j)*dtrk * ( div(k,i,j+1)-div(k,i,j) )           & ! divergence damping
...
!                                     + ATMOS_DYN_LSsink_D * CZ(k) * sink(k,i,j,5)                      & ! large scale sink
    ...
    IIE = IIS+IBLOCK-1
       ...
       do i = IIS, IIE+1
       do k = KS, KE
            div (k,i,j) = ( MOMZ_RK1(k,i,j) - MOMZ_RK1(k-1,i  ,j  ) ) * RCDZ(k) &
                        + ( MOMX_RK1(k,i,j) - MOMX_RK1(k  ,i-1,j  ) ) * RCDX(i) &
                        + ( MOMY_RK1(k,i,j) - MOMY_RK1(k  ,i  ,j-1) ) * RCDY(j)
       enddo
       enddo
       enddo
...
!                                     + ATMOS_DYN_LSsink_D * CZ(k) * sink(k,i,j,5)                      & ! large scale sink
    ...
    IIE = IIS+IBLOCK-1
       ...
       do i = IIS, IIE+1
       do k = KS, KE
            div (k,i,j) = ( MOMZ_RK2(k,i,j) - MOMZ_RK2(k-1,i  ,j  ) ) * RCDZ(k) &
                        + ( MOMX_RK2(k,i,j) - MOMX_RK2(k  ,i-1,j  ) ) * RCDX(i) &
                        + ( MOMY_RK2(k,i,j) - MOMY_RK2(k  ,i  ,j-1) ) * RCDY(j)
       enddo
       enddo
       enddo
...
call START_COLLECTION("FCT")
    ...
    IIE = IIS+IBLOCK-1
       ...
       do i = IIS-1, IIE+1
       do k = KS, KE
          QTRC_lo(k,i,j,iq) = ( QTRC(k,i,j,iq) * dens_s(k,i,j) & !* ( 1.D0 - dtrk*ATMOS_DYN_LSsink_D ) &
       ...
       enddo
          QTRC_lo(KS-1,i,j,iq) = QTRC_lo(KS,i,j,iq)
          QTRC_lo(KE+1,i,j,iq) = QTRC_lo(KE,i,j,iq)
       enddo
       enddo
       ...
       do k = KS, KE
          ...
          pjpls(k,i,j) = max( 0.D0, qflx_anti(k-1,i  ,j  ,ZDIR) ) - min( 0.D0, qflx_anti(k  ,i  ,j  ,ZDIR) ) &
          ...
          pjmns(k,i,j) = max( 0.D0, qflx_anti(k  ,i  ,j  ,ZDIR) ) - min( 0.D0, qflx_anti(k-1,i  ,j  ,ZDIR) ) &
          ...
          ! --- incoming fluxes ---
          if ( pjpls(k,i,j) > 0.D0 ) then
             rjpls(k,i,j) = dens_s(k,i,j) * ( pjmax - QTRC_lo(k,i,j,iq) ) / pjpls(k,i,j)
             if ( rjpls(k,i,j) > 1.D0 ) then
                rjpls(k,i,j) = 1.D0
             endif
          else
             rjpls(k,i,j) = 0.D0
          ...
          ! --- outgoing fluxes ---
          if ( pjmns(k,i,j) > 0.D0 ) then
             rjmns(k,i,j) = dens_s(k,i,j) * ( QTRC_lo(k,i,j,iq) - pjmin ) / pjmns(k,i,j)
    ...
    IIE = IIS+IBLOCK-1
       ...
       do k = KS-1, KE
          if ( qflx_anti(k,i,j,ZDIR) >= 0 ) then
             qflx_anti(k,i,j,ZDIR) = qflx_anti(k,i,j,ZDIR) * max( min( rjpls(k+1,i,j), rjmns(k  ,i,j), 1.D0 ), 0.D0 )
          else
             qflx_anti(k,i,j,ZDIR) = qflx_anti(k,i,j,ZDIR) * max( min( rjpls(k  ,i,j), rjmns(k+1,i,j), 1.D0 ), 0.D0 )
          endif
       enddo
       enddo
       enddo
       ...
       do k = KS, KE
          if ( qflx_anti(k,i,j,XDIR) >= 0 ) then
             qflx_anti(k,i,j,XDIR) = qflx_anti(k,i,j,XDIR) * max( min( rjpls(k,i+1,j), rjmns(k,i  ,j), 1.D0 ), 0.D0 )
          else
             qflx_anti(k,i,j,XDIR) = qflx_anti(k,i,j,XDIR) * max( min( rjpls(k,i  ,j), rjmns(k,i+1,j), 1.D0 ), 0.D0 )
          endif
       enddo
       enddo
       enddo
       ...
       do k = KS, KE
          if ( qflx_anti(k,i,j,YDIR) >= 0 ) then
             qflx_anti(k,i,j,YDIR) = qflx_anti(k,i,j,YDIR) * max( min( rjpls(k,i,j+1), rjmns(k,i,j  ), 1.D0 ), 0.D0 )
          else
             qflx_anti(k,i,j,YDIR) = qflx_anti(k,i,j,YDIR) * max( min( rjpls(k,i,j  ), rjmns(k,i,j+1), 1.D0 ), 0.D0 )
          endif
       enddo
       enddo
       enddo
       ...
       do i = IIS, IIE
       do k = KS, KE
          QTRC(k,i,j,iq) = ( QTRC_lo(k,i,j,iq) *dens_s(k,i,j) &!* ( 1.D0 - dtrk*ATMOS_DYN_LSsink_D ) &
...
!                                      + ATMOS_DYN_LSsink_D * CZ(k) * sink(k,i,j,iq+5) ) &
    ...
    IIE = IIS+IBLOCK-1
       ...
       do k = KS, KE
          ...
          ! --- STEP C: compute the total incoming and outgoing antidiffusive fluxes in each cell ---
          pjpls(k,i,j) = eps &
                       + dtrk * ( ( max( 0.D0, qflx_anti(k-1,i  ,j  ,ZDIR) ) - min( 0.D0, qflx_anti(k  ,i  ,j  ,ZDIR) ) ) * RCDZ(k)&
          ...
          pjmns(k,i,j) = eps &
                       + dtrk * ( ( max( 0.D0, qflx_anti(k  ,i  ,j  ,ZDIR) ) - min( 0.D0, qflx_anti(k-1,i  ,j  ,ZDIR) ) ) * RCDZ(k)&
...
!    call HIST_in( sink(:,:,:,5), 'sink', 'tendency large scale sink', 'kg/m3/s', '3D', TIME_DTSEC )
    call HIST_in( div (:,:,:), 'div', 'divergence', 's-1', '3D', TIME_DTSEC )
