...
module mod_atmos_phy_mp
  ...
  logical, private, save  :: doautoconversion = .true. ! Calculate autoconversion?
  logical, private, save  :: doprecipitation  = .true. ! Calculate sedimentation?
    ...
    do i = IS, IE
       ...
       do k = KS, KE
          if ( QTRC(k,i,j,I_QC) > 0.D0 ) then
             ...
             if ( qvs(k) > QTRC(k,i,j,I_QV) ) then ! unsaturatd
                vent = 1.6D0 + 30.3922D0 * ( DENS(k,i,j) * QTRC(k,i,j,I_QC) )**0.2046D0

                ! --- evaporation rate (Ogura and Takahashi, 1971) ---
                dq_evap(k) = ( qvs(k) - QTRC(k,i,j,I_QV) ) &
                           / qvs(k) * vent * ( DENS(k,i,j) * QTRC(k,i,j,I_QC) )**0.525D0   &
                           / ( DENS(k,i,j) * ( 2.03D4 + 9.584D6 / ( pres(k) * qvs(k) ) ) )
                dq_evap(k) = min( dq_evap(k), QTRC(k,i,j,I_QC)/dt )
