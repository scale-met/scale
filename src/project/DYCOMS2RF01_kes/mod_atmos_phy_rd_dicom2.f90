...
contains
  ...
  subroutine ATMOS_PHY_RD
    ...
    real(8) :: Z_cldtop(1 ,IA,JA) ! Cloud top height [m]

    real(8) :: QTOT ! Qv + Qc + Qr [kg/kg]

    real(8) :: Qbelow, Qabove ! scaled LWP (above/below layer)
    real(8) :: kappa          ! scaling factor for LWP [m2/kg]

    real(8) :: H              ! Heaviside Step Function
    real(8) :: a  = 1.D0      ! [K/m**-1/3]
    real(8) :: F0 = 70.D0     ! Upward [J/m2/s]
    real(8) :: F1 = 22.D0     ! [K/m**-1/3]
    real(8) :: D  = 3.75D-6   ! divergence of large scale horizontal winds [1/s]
    ...
    do i = IS, IE
       ...
       do k = KS, KE
          ...
          Qabove = 0.D0
          do k2 = KS, KE
             dQ = kappa * DENS(k2,i,j) * ( QTRC(k2,i,j,I_QC) + QTRC(k2,i,j,I_QR) )
    ...
    call HIST_in( Z_cldtop(:,:,:), 'Zcldtop',   'Cloud top height',       'm',         '2D', dtrd )
