...
contains
  ...
  subroutine ATMOS_PHY_TB( dens,   pott,   qtrc,                  &
    ...
    do j = JS-2, JE+2
    do i = IS-1, IE+2
       Uij(i,j,k,1,1) = ( velx(i,j,k)-velx(i-1,j,k) ) * RCDX(i) ! du/dx, (x, y, layer)
    enddo
    enddo
    enddo
    do k = KS,   KE
    do j = JS-1, JE+2
    do i = IS-2, IE+2
       Uij(i,j,k,2,2) = ( vely(i,j,k)-vely(i,j-1,k) ) * RCDY(j) ! dv/dy, (x, y, layer)
    enddo
    enddo
    enddo
    do k = KS,   KE
    do j = JS-2, JE+2
    do i = IS-2, IE+2
       Uij(i,j,k,3,3) = ( velz(i,j,k)-velz(i,j,k-1) ) * RCDZ(k) ! dw/dz, (x, y, layer)
    enddo
    enddo
    enddo
    ...
    do j = JS-2, JE+2
    do i = IS-2, IE+1
       Uij(i,j,k,1,2) = ( vely(i+1,j,k)-vely(i,j,k) ) * RCDX(i) ! dv/dx, (u, v, layer)
    enddo
    enddo
    enddo
    do k = WS+1, WE-1
    do j = JS-2, JE+2
    do i = IS-2, IE+1
       Uij(i,j,k,1,3) = ( velz(i+1,j,k)-velz(i,j,k) ) * RCDX(i) ! dw/dx, (u, y, interface)
    enddo
    enddo
    enddo
    Uij(:,:,WS,1,3) = 0.D0
    ...
    do j = JS-2, JE+1
    do i = IS-2, IE+2
       Uij(i,j,k,2,1) = ( velx(i,j+1,k)-velx(i,j,k) ) * RCDY(j) ! du/dy, (u, v, layer)
    enddo
    enddo
    enddo
    do k = WS+1, WE-1
    do j = JS-2, JE+1
    do i = IS-2, IE+2
       Uij(i,j,k,2,3) = ( velz(i,j+1,k)-velz(i,j,k) ) * RCDY(j) ! dw/dy, (x, v, interface)
    enddo
    enddo
    enddo
    Uij(:,:,WS,2,3) = 0.D0
    ...
    do j = JS-2, JE+2
    do i = IS-2, IE+2
       Uij(i,j,k,3,1) = ( velx(i,j,k+1)-velx(i,j,k) ) * RCDZ(k) ! du/dz, (u, y, interface)
       Uij(i,j,k,3,2) = ( vely(i,j,k+1)-vely(i,j,k) ) * RCDZ(k) ! dv/dz, (x, v, interface)
    enddo
    enddo
    enddo
    Uij(:,:,WS,3,1) = 0.D0
...
!    enddo
    ...
    do i = IS-1, IE
       ...
       Ri = GRAV * ( dens(i,j,k+1)-dens(i,j,k) ) * RCDZ(k) * 2.D0 / ( dens(i,j,k+1)+dens(i,j,k) )                   &
          / max( ( ( velx(i,j,k+1)+velx(i-1,j,k+1)-velx(i,j,k)-velx(i-1,j,k) ) * 0.5D0 * RCDZ(k) )**2.D0            &
               + ( ( vely(i,j,k+1)+vely(i,j-1,k+1)-vely(i,j,k)-vely(i,j-1,k) ) * 0.5D0 * RCDZ(k) )**2.D0, Ustar_min )
    ...
    do j = JS-2, JE+1
    do i = IS-2, IE+1
       nurho(i,j,k) = ( Cs * DXYZ ) ** 2.D0 * ( sqrt( 2.D0 * GAMMA                          &
    ...
    do j = JS-1, JE
    do i = IS-1, IE
       FLXt(i,j,k,1) = RPr(i,j,k) * ( pott(i,j,k)-pott(i+1,j,k) ) * RCDX(i)                             &
                     * 0.25D0 * ( nurho(i,j,k) + nurho(i,j-1,k) + nurho(i,j,k-1) + nurho(i,j-1,k-1) )
       FLXt(i,j,k,2) = RPr(i,j,k) * ( pott(i,j,k)-pott(i,j+1,k) ) * RCDY(j)                             &
                     * 0.25D0 * ( nurho(i,j,k) + nurho(i-1,j,k) + nurho(i,j,k-1) + nurho(i-1,j,k-1) )
       FLXt(i,j,k,3) = RPr(i,j,k) * ( pott(i,j,k)-pott(i,j,k+1) ) * RCDZ(k)                             &
                     * 0.25D0 * ( nurho(i,j,k) + nurho(i-1,j,k) + nurho(i,j-1,k) + nurho(i-1,j-1,k) )

       FLXq(i,j,k,1,:) = RPr(i,j,k) * ( qtrc(i,j,k,:)-qtrc(i+1,j,k,:) ) * RCDX(i)                         &
                       * 0.25D0 * ( nurho(i,j,k) + nurho(i,j-1,k) + nurho(i,j,k-1) + nurho(i,j-1,k-1) )
       FLXq(i,j,k,2,:) = RPr(i,j,k) * ( qtrc(i,j,k,:)-qtrc(i,j+1,k,:) ) * RCDY(j)                         &
                       * 0.25D0 * ( nurho(i,j,k) + nurho(i-1,j,k) + nurho(i,j,k-1) + nurho(i-1,j,k-1) )
       FLXq(i,j,k,3,:) = RPr(i,j,k) * ( qtrc(i,j,k,:)-qtrc(i,j,k+1,:) ) * RCDZ(k)                         &
                       * 0.25D0 * ( nurho(i,j,k) + nurho(i-1,j,k) + nurho(i,j-1,k) + nurho(i-1,j-1,k) )
    enddo
    enddo
    enddo
    ...
    do j = JS, JE
    do i = IS, IE
       momx_t(i,j,k) = - ( FLXij(i+1,j,k,1,1)-FLXij(i,j  ,k  ,1,1) ) * RCDX(i) &
                       + ( FLXij(i  ,j,k,1,2)-FLXij(i,j-1,k  ,1,2) ) * RCDY(j) &
                       + ( FLXij(i  ,j,k,1,3)-FLXij(i,j  ,k-1,1,3) ) * RCDZ(k)
       momy_t(i,j,k) = - ( FLXij(i,j  ,k,2,1)-FLXij(i-1,j,k  ,2,1) ) * RCDX(i) &
                       + ( FLXij(i,j+1,k,2,2)-FLXij(i  ,j,k  ,2,2) ) * RCDY(j) &
                       + ( FLXij(i,j  ,k,2,3)-FLXij(i  ,j,k-1,2,3) ) * RCDZ(k)
       momz_t(i,j,k) = - ( FLXij(i,j,k  ,3,1)-FLXij(i-1,j  ,k,3,1) ) * RCDX(i) &
                       + ( FLXij(i,j,k  ,3,2)-FLXij(i  ,j-1,k,3,2) ) * RCDY(j) &
                       + ( FLXij(i,j,k+1,3,3)-FLXij(i  ,j  ,k,3,3) ) * RCDZ(k)

       pott_t(i,j,k) = - ( ( FLXt(i,j,k,1)-FLXt(i-1,j  ,k  ,1) ) * RCDX(i) &
                         + ( FLXt(i,j,k,2)-FLXt(i  ,j-1,k  ,2) ) * RCDY(j) &
                         + ( FLXt(i,j,k,3)-FLXt(i  ,j  ,k-1,3) ) * RCDZ(k) &
                         ) / dens(i,j,k)

       qtrc_t(i,j,k,:) = - ( ( FLXq(i,j,k,1,:)-FLXq(i-1,j  ,k  ,1,:) ) * RCDX(i) &
                           + ( FLXq(i,j,k,2,:)-FLXq(i  ,j-1,k  ,2,:) ) * RCDY(j) &
                           + ( FLXq(i,j,k,3,:)-FLXq(i  ,j  ,k-1,3,:) ) * RCDZ(k) &
                           ) / dens(i,j,k)
    enddo
    enddo
    enddo
