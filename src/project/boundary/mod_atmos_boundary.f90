...
!    enddo
  ...
  subroutine ATMOS_BOUNDARY( dens,   momx,   momy,   momz,   pott,  &
    ...
    ! prognostic value
    real(8), intent(in)    :: dens(IA,JA,KA)      ! density [kg/m**3]
    real(8), intent(in)    :: momx(IA,JA,KA)      ! momentum (x) [kg/m**3 * m/s]
    real(8), intent(in)    :: momy(IA,JA,KA)      ! momentum (y) [kg/m**3 * m/s]
    real(8), intent(in)    :: momz(IA,JA,KA)      ! momentum (z) [kg/m**3 * m/s]
  ...
  subroutine ATMOS_BOUNDARY_reference_read
    ...
    enddo
    enddo
    atmos_refvar(:,:,KA-10:KA,1) = 0.D0
    atmos_refvar(1:10,:,:,1)     = 3.D0
    atmos_refvar(:,:,KA-10:KA,3) = 0.D0
    atmos_refvar(:,:,KA-10:KA,4) = 300.D0
