...
module mod_atmos_dyn
  ...
  integer, private, save :: KBLOCK, JBLOCK, IBLOCK
...
contains
  ...
  subroutine ATMOS_DYN
    ...
    ! work
    real(8) :: var_s    (KA,IA,JA,5)   ! prognostic variables (RK 1st)
    real(8) :: var_temp
...
call START_COLLECTION("SET")
    ...
    do k = KKS,   KKE
       ...
       num_diff(k,i,j,I_RHOT,ZDIR) = DIFF4 * CDZ(k)**4                      &
                                   ...
                                   * ( FACT_F * ( var(k-1,i,j,I_DENS)+var(k+2,i,j,I_DENS) ) &
                                     + FACT_N * ( var(k  ,i,j,I_DENS)+var(k+1,i,j,I_DENS) ) )
    enddo
    enddo
    enddo
    ...
    do k = KKS,   KKE
       ...
       num_diff(k,i,j,I_RHOT,XDIR) = DIFF4 * CDX(i)**4                    &
                                   ...
                                   * ( FACT_F * ( var(k,i-1,j,I_DENS)+var(k,i+2,j,I_DENS) ) &
                                     + FACT_N * ( var(k,i  ,j,I_DENS)+var(k,i+1,j,I_DENS) ) )
    enddo
    enddo
    enddo
    ...
    do k = KKS,   KKE
       ...
       num_diff(k,i,j,I_RHOT,YDIR) = DIFF4 * CDY(j)**4                    &
                                   ...
                                   * ( FACT_F * ( var(k,i,j-1,I_DENS)+var(k,i,j+2,I_DENS) ) &
                                     + FACT_N * ( var(k,i,j  ,I_DENS)+var(k,i,j+1,I_DENS) ) )
    enddo
    enddo
    enddo
    ...
    enddo ! loop blocking i
    enddo ! loop blocking j
...
!       endif
       ...
       do k = KKS-1, KKE
          diagvar(k,i,j,I_VELZ) = 2.D0 * var(k,i,j,I_MOMZ) &
                                / ( var(k  ,i,j,I_DENS)+var(k+1,i,j,I_DENS) )
       enddo
       enddo
       enddo
...
!       endif
       ...
       do i = IIS,   IIE
       do k = KKS,   KKE
          qflx_hi(k,i,j,ZDIR) = 0.25D0 * ( diagvar(k-1,i,j,I_VELZ)+diagvar(k,i,j,I_VELZ) ) &
       ...
       do i = IIS-1, IIE
       do k = KKS-1, KKE
          qflx_hi(k,i,j,XDIR) = 0.25D0 * ( diagvar(k,i,j,I_VELX)+diagvar(k+1,i,j,I_VELX) ) &
                              * ( FACT_F * ( var(k,i-1,j,I_MOMZ)+var(k,i+2,j,I_MOMZ) )     &
                                + FACT_N * ( var(k,i  ,j,I_MOMZ)+var(k,i+1,j,I_MOMZ) ) )   &
       ...
       do i = IIS,   IIE
       do k = KKS-1, KKE
          qflx_hi(k,i,j,YDIR) = 0.25D0 * ( diagvar(k,i,j,I_VELY)+diagvar(k+1,i,j,I_VELY) ) &
...
!       endif
       ...
       do k = KS,   KE-1
          var(k,i,j,I_MOMZ) = var_s(k,i,j,I_MOMZ) &
                            + dtrk * ( - ( ( -qflx_hi(k,i  ,j  ,ZDIR)+qflx_hi(k+1,i,j,ZDIR) ) * RDZF(k)   &
                                         + ( -qflx_hi(k,i-1,j  ,XDIR)+qflx_hi(k  ,i,j,XDIR) ) * RDXC(i)   &
                                         + ( -qflx_hi(k,i  ,j-1,YDIR)+qflx_hi(k  ,i,j,YDIR) ) * RDYC(j) ) & ! flux divergence
                                       - ( -diagvar(k,i,j,I_PRES)+diagvar(k+1,i,j,I_PRES) ) * RDZF(k)     & ! pressure gradient force
       ...
       do k = KS, KE
          var(k,i,j,I_DENS) = var_s(k,i,j,I_DENS) &
                            + dtrk * ( - ( ( -mflx_hi(k-1,i,j,ZDIR)+mflx_hi(k,i,j,ZDIR) ) * RDZC(k)   &
                                         + ( -mflx_hi(k,i-1,j,XDIR)+mflx_hi(k,i,j,XDIR) ) * RDXC(i)   &
                                         + ( -mflx_hi(k,i,j-1,YDIR)+mflx_hi(k,i,j,YDIR) ) * RDYC(j) ) ) ! divergence
       enddo
       enddo
       enddo
...
!       call COMM_wait( var(:,:,:,iq-1), iq-1 )
       ...
       do k = KKS, KKE
          ...
          var(k,i,j,iq) = ( var(k,i,j,iq) * var_s(k,i,j,I_DENS) &
                          + dtrk * ( - ( ( -qflx_lo(k-1,i,j,ZDIR)+qflx_lo(k,i,j,ZDIR) ) * RDZC(k)     &
                                       + ( -qflx_lo(k,i-1,j,XDIR)+qflx_lo(k,i,j,XDIR) ) * RDXC(i)     &
                                       + ( -qflx_lo(k,i,j-1,YDIR)+qflx_lo(k,i,j,YDIR) ) * RDYC(j) ) ) &
                          ) / var(k,i,j,I_DENS)
          ...
          if ( pjmns > 0 ) then
             rjmns(k,i,j,ZDIR) = var_temp / pjmns * abs((mflx_hi(k-1,i,j,ZDIR)+mflx_hi(k,i,j,ZDIR)) * 0.5D0)
             rjmns(k,i,j,XDIR) = var_temp / pjmns * abs((mflx_hi(k,i-1,j,XDIR)+mflx_hi(k,i,j,XDIR)) * 0.5D0)
             rjmns(k,i,j,YDIR) = var_temp / pjmns * abs((mflx_hi(k,i,j-1,YDIR)+mflx_hi(k,i,j,YDIR)) * 0.5D0)
          else
       ...
       do k = KKS, KKE
          var(k,i,j,iq) = var(k,i,j,iq) &
                        + dtrk * ( - ( ( -qflx_anti(k-1,i,j,ZDIR)+qflx_anti(k,i,j,ZDIR) ) * RDZC(k)   &
                                     + ( -qflx_anti(k,i-1,j,XDIR)+qflx_anti(k,i,j,XDIR) ) * RDXC(i)   &
                                     + ( -qflx_anti(k,i,j-1,YDIR)+qflx_anti(k,i,j,YDIR) ) * RDYC(j) ) &
                                 ) / var(k,i,j,I_DENS)
       enddo
       enddo
       enddo
