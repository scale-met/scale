!-------------------------------------------------------------------------------
!> Program Rayleigh Damping Test for SCALE-LES ver.3
!!
!! @par Description
...
!    real(8) :: ZR_BBL = 2.D3  ! bubble radius   [m]: z
    ...
    NAMELIST / PARAM_MKEXP_sonicanddamp / &
       ENV_THETA
!       XC_BBL,    &
!       YC_BBL,    &
!       ZC_BBL,    &
...
!    real(8) :: dist
    ...
    if( IO_L ) write(IO_FID_LOG,*)
    if( IO_L ) write(IO_FID_LOG,*) '+++ Module[sonicanddamp]/Categ[INIT]'

    !--- read namelist
    rewind(IO_FID_CONF)
    read(IO_FID_CONF,nml=PARAM_MKEXP_sonicanddamp,iostat=ierr)

    if( ierr < 0 ) then !--- missing
       if( IO_L ) write(IO_FID_LOG,*) '*** Not found namelist. Default used.'
    elseif( ierr > 0 ) then !--- fatal error
       write(*,*) 'xxx Not appropriate names in namelist PARAM_MKEXP_sonicanddamp. Check!'
       call PRC_MPIstop
    endif
    if( IO_L ) write(IO_FID_LOG,nml=PARAM_MKEXP_sonicanddamp)
...
!       endif
  ...
  end subroutine MKEXP_sonicanddamp

end program sonicanddamp
