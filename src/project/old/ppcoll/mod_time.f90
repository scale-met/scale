...
module mod_time
  ...
  public :: PAPI_report      ! 2012/02/22 R.Yoshida [add]
  ...
  !-----------------------------------------------------------------------------
  !
  !++ PAPI variables ( 2012/02/22 R.Yoshida [add] )
  !
  integer,    public, save :: PAPI_check
  integer(8), public, save :: PAPI_flpins
  real(4),    public, save :: PAPI_real_time
  real(4),    public, save :: PAPI_proc_time
  real(4),    public, save :: PAPI_mflops
  real(8),    public, save :: PAPI_mflins
...
contains
  ...
  !-----------------------------------------------------------------------------
  subroutine PAPI_report                        ! 2012/02/22 R.Yoshida [add]
    implicit none
    ...
    if( IO_L ) write(IO_FID_LOG,*)
    if( IO_L ) write(IO_FID_LOG,*) '*** PAPI FLOPS/MIPS Report'

    papi_mflins = dble(papi_flpins)/1.0d6

    if( IO_L ) write(IO_FID_LOG,'(1x,A,F10.3,A,F10.3)') &
                                '*** Real Time:',PAPI_real_time, &
                                ' Proc Time:',PAPI_proc_time
    if( IO_L ) write(IO_FID_LOG,'(1x,A,F10.3,A,F10.3)') &
                                '*** MFLIPS:',PAPI_mflins, &
                                ' MFLOPS:',PAPI_mflops
