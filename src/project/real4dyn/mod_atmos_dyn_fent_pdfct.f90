...
module mod_atmos_dyn
  ...
  real(4), allocatable, save :: CDX(:)
  real(4), allocatable, save :: CDY(:)
  real(4), allocatable, save :: CDZ(:)
  real(4), allocatable, save :: RDXC(:)
  real(4), allocatable, save :: RDYC(:)
  real(4), allocatable, save :: RDZC(:)
  real(4), allocatable, save :: RDXF(:)
  real(4), allocatable, save :: RDYF(:)
  real(4), allocatable, save :: RDZF(:)
...
contains
  ...
  subroutine ATMOS_DYN_setup
    ...
    CDX(:)  = real(GRID_CDX(:),  kind=4)
    CDY(:)  = real(GRID_CDY(:),  kind=4)
    CDZ(:)  = real(GRID_CDZ(:),  kind=4)
    RDXC(:) = real(GRID_RCDX(:), kind=4)
    RDYC(:) = real(GRID_RCDY(:), kind=4)
    RDZC(:) = real(GRID_RCDZ(:), kind=4)
    RDXF(:) = real(GRID_RFDX(:), kind=4)
    RDYF(:) = real(GRID_RFDY(:), kind=4)
    RDZF(:) = real(GRID_RFDZ(:), kind=4)
  ...
  subroutine ATMOS_DYN
    ...
    real(4) :: REF_pott  (KA)
    ! work
    real(4) :: var_s    (KA,IA,JA,VA)  ! prognostic variables (previous step)
    ...
    var(:,:,:,:)        = real(atmos_var(:,:,:,:), kind=4)
    REF_dens(:)         = real(ATMOS_REFSTATE_dens(:), kind=4)
    REF_pott(:)         = real(ATMOS_REFSTATE_pott(:), kind=4)
    DAMP_var(:,:,:,:)   = real(ATMOS_BOUNDARY_var(:,:,:,:), kind=4)
    DAMP_alpha(:,:,:,:) = real(ATMOS_BOUNDARY_alpha(:,:,:,:), kind=4)
...
!       if ( rko > 1 ) then
!          call COMM_wait_r4( var(:,:,:,I_DENS), I_DENS )
!          call COMM_wait_r4( var(:,:,:,I_MOMZ), I_MOMZ )
!          call COMM_wait_r4( var(:,:,:,I_MOMX), I_MOMX )
!          call COMM_wait_r4( var(:,:,:,I_MOMY), I_MOMY )
!       endif
...
!          call COMM_vars_r4( mflx_hi(:,:,:,ZDIR), VA+ZDIR )
!          call COMM_vars_r4( mflx_hi(:,:,:,XDIR), VA+XDIR )
!          call COMM_vars_r4( mflx_hi(:,:,:,YDIR), VA+YDIR )
!       endif
...
!       if ( rko > 1 ) then
!          call COMM_wait_r4( var(:,:,:,I_RHOT), I_RHOT )
!       endif
...
!       call COMM_vars_r4( var(:,:,:,I_DENS), I_DENS )
!       call COMM_vars_r4( var(:,:,:,I_MOMZ), I_MOMZ )
...
!       call COMM_vars_r4( var(:,:,:,I_MOMX), I_MOMX )
...
!       call COMM_vars_r4( var(:,:,:,I_MOMY), I_MOMY )
...
!       call COMM_vars_r4( var(:,:,:,I_RHOT), I_RHOT )

       call COMM_vars_r4( var(:,:,:,1), 1 )
       call COMM_vars_r4( var(:,:,:,2), 2 )
       call COMM_vars_r4( var(:,:,:,3), 3 )
       call COMM_vars_r4( var(:,:,:,4), 4 )
       call COMM_vars_r4( var(:,:,:,5), 5 )
    ...
    call COMM_vars_r4( mflx_hi(:,:,:,XDIR), VA+XDIR )
    call COMM_vars_r4( mflx_hi(:,:,:,YDIR), VA+YDIR )
    call COMM_wait_r4( mflx_hi(:,:,:,ZDIR), VA+ZDIR )
    call COMM_wait_r4( mflx_hi(:,:,:,XDIR), VA+XDIR )
    call COMM_wait_r4( mflx_hi(:,:,:,YDIR), VA+YDIR )
...
!       call COMM_wait_r4( var(:,:,:,iq-1), iq-1 )
       ...
       do k = KS, KE
          ...
          if ( pjmns > 0 ) then
             rjmns(k,i,j,ZDIR) = var_s(k,i,j,iq) / pjmns * abs((mflx_hi(k,i,j,ZDIR)+mflx_hi(k-1,i  ,j  ,ZDIR)) * 0.5D0)
             rjmns(k,i,j,XDIR) = var_s(k,i,j,iq) / pjmns * abs((mflx_hi(k,i,j,XDIR)+mflx_hi(k  ,i-1,j  ,XDIR)) * 0.5D0)
             rjmns(k,i,j,YDIR) = var_s(k,i,j,iq) / pjmns * abs((mflx_hi(k,i,j,YDIR)+mflx_hi(k  ,i  ,j-1,YDIR)) * 0.5D0)
          else
